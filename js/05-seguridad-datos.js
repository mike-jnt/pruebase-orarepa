(() => {
  ...
  const HARDENING_VERSION = '2026.08.08-C9.29';
  ...
  async function diagnosticarPermisosFirestoreC94(mostrarAviso = false) {
    ...
    console.info('[Señor Arepa] Diagnóstico Firestore C9.29', resultado);
    if (mostrarAviso) {
      notify(
        correcto
          ? `Permisos Firestore C9.29 verificados para ${email}.`
          : `Las reglas C9.29 no están activas para ${email || 'la cuenta actual'}. Publica firestore.rules antes de continuar.`,
  ...
  async function reconcileDailyCounter(day, sales = []) {
    ...
    const list = (Array.isArray(sales) ? sales : []).map(item => secureNormalizarVenta(item || {}));
    const total = list.length;
    const maxCommand = list.reduce((max, candidate) => Math.max(max, positiveConsecutive(candidate.numeroVentaDia), positiveConsecutive(candidate.comanda)), 0);
    const maxReceipt = list.reduce((max, candidate) => Math.max(max, positiveConsecutive(candidate.numeroVentaDia), positiveConsecutive(candidate.recibo)), 0);
    const maxDelivery = list.reduce((max, candidate) => Math.max(max, positiveConsecutive(candidate.numeroDomicilio)), 0);
    ...
    try {
      ...
      await firestoreDb.runTransaction(async transaction => {
        ...
        const command = Math.max(
          positiveConsecutive(current.comanda),
          positiveConsecutive(current.numeroVentaDia),
          positiveConsecutive(current.recibo),
          total,
          maxCommand,
          maxReceipt
        );
        const receipt = command;
        const delivery = Math.max(positiveConsecutive(current.domicilio), maxDelivery);
        result = { command, receipt, delivery };
        const needsUpdate = !snap.exists
          || command !== positiveConsecutive(current.comanda)
          || command !== positiveConsecutive(current.recibo)
          || delivery !== positiveConsecutive(current.domicilio)
          || command !== positiveConsecutive(current.numeroVentaDia);
        ...
        transaction.set(ref, {
          ...
          ordenDia: command,
          domicilio: delivery,
          esquemaConsecutivo: 'diario_unico_v2',
          reconciliadoConVentas: total,
          actualizadoEnCliente: new Date().toISOString(),
          actualizadoServidor: fieldValueServerTimestamp()
        }, { merge: true });
      });
      if (result && result.command >= total) {
        console.info(`[C9.29] Contador diario reconciliado ${day}: base ${result.command} para ${total} venta(s).`);
      }
      return result;
    } catch (error) {
      console.warn(`[C9.29] No se pudo reconciliar el contador diario ${day}:`, error);
      return null;
    }
  }
  ...
  async function executeSaleOperation(item) {
    ...
    // C9.29: el día de negocio se fija al crear la venta y nunca se vuelve a
    ...
    const localSeed = localDailyConsecutiveSeed(saleDayKey, saleIdentity(sale));
    await firestoreDb.runTransaction(async transaction => {
      ...
      const needsOfficialConsecutive = item.tipo !== 'delete' && !remote && (
        Boolean(sale.consecutivoTemporal)
        || positiveConsecutive(sale.numeroVentaDia) === 0
        || positiveConsecutive(sale.comanda) === 0
      );
      if (needsOfficialConsecutive) {
        counterRef = firestoreDb.collection('contadores').doc(firestoreDocId(saleDayKey));
        dailySummaryRef = firestoreDb.collection('resumenVentasDiario').doc(firestoreDocId(saleDayKey));
        counterSnap = await transaction.get(counterRef);
        dailySummarySnap = await transaction.get(dailySummaryRef);
      }
      ...
      if (needsOfficialConsecutive) {
        ...
        const dailySummary = dailySummarySnap?.exists ? dailySummarySnap.data() : {};
        const summaryCount = summarySaleCount(dailySummary);
        const summaryDomicilios = summaryDeliveryCount(dailySummary);

        // C9.29: si el contador fue creado después de que ya existieran ventas,
        // se eleva automáticamente hasta el total real conocido. Esto evita el
        // caso 48 ventas / comanda máxima 16 producido por numeración local en
        // distintos equipos. La transacción mantiene la unicidad entre cajas.
        const commandBase = Math.max(
          Number(counters?.comanda || counters?.numeroVentaDia || 0),
          Number(localSeed.maxCommand || 0),
          Number(localSeed.total || 0),
          Number(summaryCount || 0)
        );
        // Desde C9.29 venta, comanda y recibo comparten el mismo consecutivo.
        // No existe un segundo contador independiente que pueda desalinearse.
        const deliveryBase = Math.max(
          Number(counters?.domicilio || 0),
          Number(localSeed.maxDelivery || 0),
          Number(summaryDomicilios || 0)
        );
        ...
        officialFields = {
          ...
          consecutivoOrigen: 'firestore_transaccion_diaria_v2',
      ...
      for (const original of [...queue]) {
        ...
        try {
          ...
          const detailedMessage = permissionDenied
            ? `${message} Cuenta autenticada: ${firebaseAuth?.currentUser?.email || 'sin correo'}. Publica firestore.rules C9.29: esta versión habilita la lectura GET de movimientosInventario que la transacción usa para impedir descuentos duplicados.`
            : message;
          queue = queue.map(item => item.operacionId === original.operacionId
...
console.info('[Señor Arepa] Código activo: C9.29 · proyecto esperado: prsenorarepa');
