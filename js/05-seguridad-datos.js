(() => {
  ...
  const HARDENING_VERSION = '2026.09.06-C9.39';
  ...
  const EXCLUSIVE_SESSION_KEY = 'senorArepaExclusiveSessionIdV1';
  ...
  // C9.39: la seguridad operativa usa los usuarios realmente configurados
  ...
  function isOnlineFirebase() {
    // C9.39: no usamos firestoreDisponible como bloqueo absoluto. Ese indicador
  ...
  async function diagnosticarPermisosFirestoreC94(mostrarAviso = false) {
    ...
    console.info('[Señor Arepa] Diagnóstico Firestore C9.39', resultado);
    if (mostrarAviso) {
      notify(
        correcto
          ? `Permisos Firestore C9.39 verificados para ${email}.`
          : `Las reglas C9.39 no están activas para ${email || 'la cuenta actual'}. Publica firestore.rules antes de continuar.`,
  ...
  function buildJornadaId(day, openingISO = '') {
    const safeDay = cleanText(day, 20) || colombiaDayKey(new Date());
    const openingDate = timestampToDate(openingISO);
    const stamp = openingDate ? openingDate.getTime() : 0;
    return `J_${safeDay}_${stamp || 'sin_apertura'}`;
  }

  async function resolveBusinessJornada(day, suppliedControl = null) {
    const safeDay = cleanText(day, 20) || colombiaDayKey(new Date());
    let control = suppliedControl && typeof suppliedControl === 'object' ? suppliedControl : null;
    if (!control && isOnlineFirebase()) {
      try {
        const snap = await firestoreDb.collection('controlCaja').doc(firestoreDocId(safeDay)).get();
        control = snap.exists ? (snap.data() || {}) : {};
      } catch (error) {
        console.warn(`[C9.39] No se pudo leer la apertura de caja de ${safeDay} para preparar la jornada:`, error);
        control = {};
      }
    }
    control = control || {};
    const openingDate = timestampToDate(control.jornadaInicioISO || control.aperturaHora);
    const openingISO = openingDate ? openingDate.toISOString() : '';
    const jornadaId = cleanText(control.jornadaId || buildJornadaId(safeDay, openingISO), 120);
    return {
      diaClave: safeDay,
      jornadaId,
      jornadaInicioISO: openingISO,
      jornadaInicioMs: openingDate ? openingDate.getTime() : 0,
      aperturaRegistrada: Boolean(openingISO),
      aperturaMonto: Math.max(0, Number(control.aperturaMonto || 0))
    };
  }

  window.obtenerJornadaIdCaja = function(day, openingISO = '') {
    return buildJornadaId(day, openingISO);
  };
  ...
  function getExclusiveSessionId() {
    try {
      return cleanText(sessionStorage.getItem(EXCLUSIVE_SESSION_KEY) || '', 150);
    } catch (_) {
      return '';
    }
  }

  function requireExclusiveSessionId() {
    const sessionId = getExclusiveSessionId();
    if (!sessionId) {
      const error = new Error('No existe una sesión POS exclusiva válida para sincronizar la venta.');
      error.code = 'exclusive-session-missing';
      throw error;
    }
    return sessionId;
  }
  ...
  // ---------------------------------------------------------------------------
  function validBusinessDayKey(value = '') {
    return /^\d{4}-\d{2}-\d{2}$/.test(String(value || ''));
  }

  function immutableSaleDayKey(sale = {}, operationCreatedAt = '') {
    const explicit = cleanText(sale?.diaClave || sale?.diaClaveOficial || sale?.fechaNegocioDia || '', 20);
    if (validBusinessDayKey(explicit)) return explicit;
    // C9.39: una cola heredada sin diaClave solo puede recuperar el día desde
    // fechas que ya pertenecían a ESA venta/operación. Nunca usamos la fecha
    // actual porque eso trasladaría pedidos viejos a una jornada nueva.
    const sourceDate = timestampToDate(
      sale?.fechaNegocioISO || sale?.fechaISO || sale?.fechaOficialISO || sale?.fecha || operationCreatedAt
    );
    return sourceDate ? colombiaDayKey(sourceDate) : '';
  }
  ...
  function readSalesQueue() {
    ...
    return data.map((item, index) => {
      const creadoEnOriginal = cleanText(item?.creadoEn || '', 50);
      const sale = secureNormalizarVenta(item?.venta || {});
      const diaInmutable = immutableSaleDayKey(sale, creadoEnOriginal);
      if (diaInmutable) {
        sale.diaClave = diaInmutable;
        sale.diaClaveOficial = sale.diaClaveOficial || diaInmutable;
        sale.mesClave = sale.mesClave || diaInmutable.slice(0, 7);
      } else {
        sale.requiereRevisionDia = true;
      }
      return {
        ...
        bloqueoPermanente: Boolean(item?.bloqueoPermanente),
        codigoError: cleanText(item?.codigoError || '', 80),
        ...
        sessionId: cleanText(item?.sessionId || '', 150)
      };
  ...
  guardarVentaPendienteSync = function(sale, inventoryAdjustments = {}, type = 'set', options = {}) {
    ...
    const creadoEnOperacion = options.creadoEn || new Date().toISOString();
    const diaInmutable = immutableSaleDayKey(normalizedSale, creadoEnOperacion);
    if (type !== 'delete' && !diaInmutable) {
      throw new Error('La venta no tiene una fecha de negocio verificable. No se guardará en la cola para evitar asignarla a una jornada incorrecta.');
    }
    if (diaInmutable) {
      normalizedSale.diaClave = diaInmutable;
      normalizedSale.diaClaveOficial = normalizedSale.diaClaveOficial || diaInmutable;
      normalizedSale.mesClave = normalizedSale.mesClave || diaInmutable.slice(0, 7);
    }
    ...
    const operation = {
      ...
      sessionId: cleanText(options.sessionId || getExclusiveSessionId(), 150)
    };
  ...
  function inventoryAliasCandidates(value = '') {
    const baseKey = normalizeKey(value || '');
    const set = new Set([baseKey]);
    if (!baseKey) return [];
    // C9.39: normalizeKey elimina espacios; los alias deben trabajar sobre la
    // clave compacta real que se guarda en nombreNormalizado/aliasNombres.
    // Ejemplo: COCACOLA400 <-> COCACOLA400ML.
    if (/\d+$/.test(baseKey)) set.add(`${baseKey}ML`);
    if (/\d+ML$/.test(baseKey)) set.add(baseKey.replace(/ML$/, ''));
    if (/\d+L$/.test(baseKey)) set.add(baseKey.replace(/L$/, ''));
    return Array.from(set).filter(Boolean);
  }

  async function resolveInventoryReference(key, adjustment, options = {}) {
    const allowMissing = Boolean(options.allowMissing);
    const aliases = new Set([
      ...inventoryAliasCandidates(adjustment?.legacyKey || adjustment?.nombre || ''),
      ...inventoryAliasCandidates(key || '')
    ]);
    try {
      const localInventory = JSON.parse(localStorage.getItem(INVENTARIO_STORAGE_KEY) || '[]');
      const cached = localInventory.find(item => {
        const itemId = String(item?.inventoryId || '');
        const itemDoc = String(item?._docId || item?.id || '');
        const itemName = normalizeKey(item?.nombreNormalizado || item?.nombre || '');
        return itemId === String(key) || itemDoc === String(key) || aliases.has(itemName);
      });
    ...
    for (const alias of aliases) {
      let legacy = await firestoreDb.collection(INVENTARIO_COLLECTION).where('nombreNormalizado', '==', alias).limit(1).get();
      if (!legacy.empty) return legacy.docs[0].ref;
      // C9.39: al renombrar una bebida desde Editar menú se conserva el nombre
      // anterior como alias. Esto permite procesar ventas históricas que todavía
      // solo almacenaban el texto antiguo y no inventoryId/productId.
      legacy = await firestoreDb.collection(INVENTARIO_COLLECTION).where('aliasNombres', 'array-contains', alias).limit(1).get();
      if (!legacy.empty) return legacy.docs[0].ref;
    }
  ...
  function saleSummaryContribution(value) {
    ...
    const day = cleanText(immutableSaleDayKey(sale, ''), 20);
    if (!validBusinessDayKey(day)) return null;
  ...
  // C9.39: el contador diario deja de "adivinar" ventas desde localStorage o desde
  // la cantidad de documentos. Se migra usando consecutivos oficiales de Firestore y,
  ...
  async function fetchConfirmedSalesForCounter(day, includeLegacyScan = false, jornada = null) {
    ...
    const addDocument = doc => {
      ...
      const rawDate = timestampToDate(
        sale.fechaNegocioISO || sale.fechaISO || sale.fechaOficialISO || sale.fechaServidor || sale.fecha
      );
      const saleDay = cleanText(
        sale.diaClave || sale.diaClaveOficial || (rawDate ? colombiaDayKey(rawDate) : ''),
        20
      );
      if (saleDay !== day) return;

      // C9.39: el contador se limita a la jornada abierta. Los documentos que
      // tengan jornadaId deben coincidir exactamente. Los documentos heredados
      // sin jornadaId solo se consideran si su fecha cae después de la apertura.
      if (jornada?.jornadaId) {
        const saleJornadaId = cleanText(sale.jornadaId || '', 120);
        if (saleJornadaId) {
          if (saleJornadaId !== jornada.jornadaId) return;
        } else if (jornada.jornadaInicioMs) {
          // Si un documento heredado no tiene jornadaId, solo puede afectar el
          // contador actual cuando podemos demostrar por fecha que fue creado
          // después de la apertura. Un registro sin fecha no adelanta comandas.
          if (!rawDate || rawDate.getTime() < jornada.jornadaInicioMs) return;
        }
      }
    ...
    // completa de compatibilidad. El filtro de jornada se conserva.
  ...
  async function prepareAtomicDailyCounter(day, options = {}) {
    ...
    const sessionId = requireExclusiveSessionId();
    const force = Boolean(options.force);
    const jornada = options.jornada || await resolveBusinessJornada(day, options.control || null);
    if (!jornada.aperturaRegistrada) {
      throw new Error(`La caja del ${day} aún no tiene una apertura sincronizada; el contador no se inicializará antes de tiempo.`);
    }
    const cacheKey = `${day}|${jornada.jornadaId}`;
    const cached = atomicCounterPreparation.get(cacheKey);
    ...
    const controlRef = firestoreDb.collection('controlCaja').doc(firestoreDocId(day));
    const preCounterSnap = await ref.get();
    const preCounter = preCounterSnap.exists ? (preCounterSnap.data() || {}) : {};
    const preAtomic = String(preCounter.esquemaConsecutivo || '') === 'atomico_v4'
      && Number(preCounter.schemaVersionContador || 0) >= 4;
    const preSameJornada = cleanText(preCounter.jornadaId || '', 120) === jornada.jornadaId;

    // C9.39: un contador atómico solo puede reutilizarse si pertenece a la
    // apertura de caja actual. Si el documento quedó con una jornada anterior
    // o sin jornadaId, se reconstruye con las ventas reales de la apertura actual.
    if (preAtomic && preSameJornada && !force) {
      const result = {
        day,
        jornadaId: jornada.jornadaId,
        jornadaInicioISO: jornada.jornadaInicioISO,
      ...
      atomicCounterPreparation.set(cacheKey, { at: Date.now(), result });
      return result;
    }

    const suppliedSales = Array.isArray(options.sales)
      ? options.sales.map(item => secureNormalizarVenta(item || {})).filter(item => {
          if (!jornada?.jornadaId) return true;
          const itemJornada = cleanText(item?.jornadaId || '', 120);
          if (itemJornada) return itemJornada === jornada.jornadaId;
          const itemDate = timestampToDate(item?.fechaNegocioISO || item?.fechaISO || item?.fechaOficialISO || item?.fecha);
          return !jornada.jornadaInicioMs || (itemDate && itemDate.getTime() >= jornada.jornadaInicioMs);
        })
      : null;
    const sales = suppliedSales || await fetchConfirmedSalesForCounter(day, true, jornada);
    const confirmedDocs = sales.length;
    let result = null;

    await firestoreDb.runTransaction(async transaction => {
      const controlSnap = await transaction.get(controlRef);
      const txControl = controlSnap.exists ? (controlSnap.data() || {}) : {};
      const txOpeningDate = timestampToDate(txControl.jornadaInicioISO || txControl.aperturaHora);
      const txOpeningISO = txOpeningDate ? txOpeningDate.toISOString() : '';
      const txJornadaId = cleanText(txControl.jornadaId || buildJornadaId(day, txOpeningISO), 120);
      if (!txOpeningISO || txJornadaId !== jornada.jornadaId) {
        const error = new Error('La apertura de caja cambió mientras se preparaba el contador. Se reintentará con la jornada vigente.');
        error.code = 'jornada-changed';
        throw error;
      }

      const snap = await transaction.get(ref);
      const current = snap.exists ? (snap.data() || {}) : {};
      const alreadyAtomic = String(current.esquemaConsecutivo || '') === 'atomico_v4'
        && Number(current.schemaVersionContador || 0) >= 4;
      const sameJornada = cleanText(current.jornadaId || '', 120) === jornada.jornadaId;

      const currentExisting = Math.max(0, Number(current.ventasExistentes || 0));
      const preserveProgress = alreadyAtomic && sameJornada && (
        !force
        || currentExisting > confirmedDocs
        || Boolean(current.ultimaVentaId)
      );

      // Nunca se retrocede un contador de la MISMA jornada que ya haya emitido
      // una venta. Esto protege una carrera entre la apertura y la primera venta.
      if (preserveProgress) {
        result = {
          day,
          jornadaId: jornada.jornadaId,
      ...
      // La base se calcula EXCLUSIVAMENTE con ventas pertenecientes a la jornada
      // actual. Si la apertura acaba de comenzar y no existen ventas, queda en 0
      // y la primera venta recibe comanda 1.
      const sequenceOf = candidate => Math.max(
      ...
      const ventasConConsecutivo = sales.filter(candidate => sequenceOf(candidate) > 0);
      const ventasSinConsecutivo = sales.filter(candidate => sequenceOf(candidate) === 0);
      const maxConfirmedSequence = ventasConConsecutivo.reduce((max, candidate) => Math.max(max, sequenceOf(candidate)), 0);
      // C9.39: la cantidad de documentos jamás se convierte en número de
      // comanda. Cuatro documentos sin consecutivo NO significan comandas 1..4.
      // Si no existe un número oficial de esta jornada, la base es 0.
      const baseSequence = maxConfirmedSequence;
      ...
      result = {
        ...
        ventasEmitidas: ventasConConsecutivo.length,
        ventasExistentes: confirmedDocs,
        documentosConfirmadosObservados: confirmedDocs,
        documentosSinConsecutivo: ventasSinConsecutivo.length,
        migrated: true,
        atomic: true
      };
      ...
      transaction.set(ref, {
        ...
        documentosConfirmadosObservados: confirmedDocs,
        documentosSinConsecutivo: ventasSinConsecutivo.length,
        requiereRevisionDocumentosSinConsecutivo: ventasSinConsecutivo.length > 0,
        ...
        jornadaAnteriorDetectada: cleanText(current.jornadaId || '', 120),
        actualizadoEnCliente: new Date().toISOString(),
        actualizadoServidor: fieldValueServerTimestamp()
      }, { merge: true });
    });

    // El cache anterior del mismo día no puede sobrevivir a un cambio de jornada.
    for (const key of Array.from(atomicCounterPreparation.keys())) {
      if (String(key).startsWith(`${day}|`) && key !== cacheKey) atomicCounterPreparation.delete(key);
    }
    atomicCounterPreparation.set(cacheKey, { at: Date.now(), result });
    if (result?.migrated) {
      console.info(`[C9.39] Contador ${day} preparado para ${result.jornadaId}: base oficial ${result.ultimoConsecutivo}; ${result.documentosConfirmadosObservados} documento(s), ${result.documentosSinConsecutivo || 0} sin consecutivo.`);
    }
    return result;
  }

  async function initializeCounterForNewJornada(day, control = null) {
    const jornada = await resolveBusinessJornada(day, control);
    if (!jornada.aperturaRegistrada) {
      throw new Error('No existe una apertura de caja válida para inicializar la jornada.');
    }
    for (const key of Array.from(atomicCounterPreparation.keys())) {
      if (String(key).startsWith(`${day}|`)) atomicCounterPreparation.delete(key);
    }
    return prepareAtomicDailyCounter(day, { force: true, jornada, control });
  }
  window.inicializarContadorNuevaJornada = initializeCounterForNewJornada;

  window.diagnosticarContadorJornada = async function(day = colombiaDayKey(new Date())) {
    const jornada = await resolveBusinessJornada(day);
    const ref = firestoreDb.collection('contadores').doc(firestoreDocId(day));
    const snap = await ref.get();
    const contador = snap.exists ? (snap.data() || {}) : {};
    const ventasJornada = await fetchConfirmedSalesForCounter(day, true, jornada);
    const numeros = ventasJornada.map(v => positiveConsecutive(v.numeroVentaDia || v.ordenDia || v.comanda || v.recibo)).filter(Boolean);
    const maximoOficial = numeros.length ? Math.max(...numeros) : 0;
    let contadoresLocalesLegacy = [];
    try {
      contadoresLocalesLegacy = Object.keys(localStorage).filter(key => /^numeracionDia_/i.test(key));
    } catch (_) {}
    const resultado = {
      version: 'C9.39',
      fuenteNumeracion: 'Firestore contadores/{diaClave}.ultimoConsecutivo',
      localStorageNumeracionIgnorado: true,
      contadoresLocalesLegacyDetectados: contadoresLocalesLegacy.length,
      diaClave: day,
      jornadaId: jornada.jornadaId,
      jornadaInicioISO: jornada.jornadaInicioISO,
      aperturaRegistrada: jornada.aperturaRegistrada,
      ventasRealesJornada: ventasJornada.length,
      ventasSinConsecutivoOficial: Math.max(0, ventasJornada.length - numeros.length),
      maximoConsecutivoJornada: maximoOficial,
      contadorJornadaId: cleanText(contador.jornadaId || '', 120),
      ultimoConsecutivoContador: positiveConsecutive(contador.ultimoConsecutivo || contador.numeroVentaDia || contador.comanda),
      siguienteEsperado: maximoOficial + 1
    };
  ...
  function buildInventoryAdjustmentsForConfirmedSale(sale, fallback = {}) {
    ...
    try {
      ...
      console.warn('[C9.39] No se pudieron reconstruir ajustes de inventario:', error);
    }
    return {};
  }

  function isPermanentInventoryWarning(error) {
    const message = String(error?.message || error || '').toLowerCase();
    return message.includes('stock insuficiente') || message.includes('inventario inexistente');
  }
  ...
  async function postProcessConfirmedSale(saleInput, operationId, inventoryAdjustments = {}) {
    ...
    // C9.39: resúmenes + marcador idempotente se confirman ANTES del inventario.
    // Un producto sin vínculo de inventario nunca volverá a bloquear Históricos,
    // caja o Finanzas. El marcador evita aplicar los resúmenes dos veces.
    await firestoreDb.runTransaction(async transaction => {
    ...
    if (!saleSnap.exists) throw new Error(`La venta ${docId} desapareció antes de completar inventario.`);
    const remoteSale = secureNormalizarVenta({ _docId: saleSnap.id, ...saleSnap.data() });
    const adjustments = buildInventoryAdjustmentsForConfirmedSale(remoteSale, inventoryAdjustments);
    const warnings = [];
    const transientErrors = [];

    // C9.39: inventario se procesa de forma independiente e idempotente por
    // producto. Si un producto antiguo no tiene inventario vinculado, se omite
    // con advertencia y NO se vuelve a reintentar indefinidamente.
    for (const [key, adjustment] of Object.entries(adjustments || {})) {
      let inventoryRef = null;
      try {
        inventoryRef = await resolveInventoryReference(key, adjustment, { allowMissing: true });
      } catch (error) {
        transientErrors.push(error);
        continue;
      }
      if (!inventoryRef) {
        warnings.push(`Inventario no vinculado: ${cleanText(adjustment?.nombre || key, 120)}`);
        continue;
      }

      const movementRef = firestoreDb.collection('movimientosInventario').doc(firestoreDocId(`${opId}_${key}`));
      try {
        await firestoreDb.runTransaction(async transaction => {
          const movementSnap = await transaction.get(movementRef);
          if (movementSnap.exists) return;
          const inventorySnap = await transaction.get(inventoryRef);
          if (!inventorySnap.exists) throw new Error(`Inventario inexistente para ${adjustment?.nombre || key}.`);
          const current = Number(inventorySnap.data()?.cantidad || 0);
          const delta = Number(adjustment?.cantidad || 0);
          const next = current + delta;
          if (next < 0) throw new Error(`Stock insuficiente para ${adjustment?.nombre || key}. Disponible: ${current}.`);
          const timestamp = fieldValueServerTimestamp();
          transaction.update(inventoryRef, {
            cantidad: next,
            inventoryId: key,
            productId: adjustment?.productId || '',
            nombreNormalizado: adjustment?.legacyKey || normalizeKey(adjustment?.nombre),
            fechaISOCliente: new Date().toISOString(),
            actualizadoServidor: timestamp
          });
          transaction.set(movementRef, {
            ...
            ventaId: docId,
            tipoOperacion: 'set',
            producto: cleanText(adjustment?.nombre || key, 150),
            productId: cleanText(adjustment?.productId || '', 100),
            inventoryId: key,
            nombreNormalizado: adjustment?.legacyKey || normalizeKey(adjustment?.nombre),
        ...
        if (isPermanentInventoryWarning(error)) {
          warnings.push(cleanText(error?.message || error, 220));
        } else {
          transientErrors.push(error);
        }
      }
    ...
    if (transientErrors.length) {
      const message = cleanText(transientErrors.map(error => error?.message || String(error)).join(' | '), 450);
      await saleRef.update({
        postProcesoEstado: 'pendiente',
        postProcesoError: message,
        postProcesoActualizadoServidor: fieldValueServerTimestamp()
      });
      throw transientErrors[0];
    }

    const warningText = cleanText(warnings.join(' | '), 450);
    await saleRef.update({
      postProcesoEstado: 'completo',
      postProcesoError: warningText,
      postProcesoActualizadoServidor: fieldValueServerTimestamp()
    });
  ...
  async function syncPendingSalePostprocesses() {
    if (syncingPostprocesosVenta || !isOnlineFirebase()) return { completas: 0, advertencias: 0, fallidas: 0 };
    ...
    try {
      // C9.39: vacía progresivamente el atasco histórico. Antes siempre se
      // consultaban las mismas primeras 25 ventas y un error permanente podía
      // impedir que las siguientes llegaran a procesarse.
      for (let batch = 0; batch < 5; batch += 1) {
        const snapshot = await firestoreDb.collection('ventas')
          .where('postProcesoEstado', '==', 'pendiente')
          .limit(50)
          .get();
        const docs = snapshot.docs.filter(doc => !processed.has(doc.id));
        ...
        for (const doc of docs) {
          ...
          const sale = secureNormalizarVenta({ _docId: doc.id, ...doc.data() });
          try {
            const result = await postProcessConfirmedSale(sale, sale.postProcesoOperacionId || `post_${doc.id}`);
            completas += 1;
            progress += 1;
            if (result?.warnings?.length) {
              advertencias += 1;
              console.warn(`[C9.39] Venta #${sale.numeroVentaDia || '?'} completada con advertencia de inventario: ${result.warnings.join(' | ')}`);
            }
          } catch (error) {
            fallidas += 1;
            console.warn(`[C9.39] Venta #${sale.numeroVentaDia || '?'} conserva postproceso pendiente por error transitorio: ${cleanText(error?.message || error, 300)}`);
            try {
        ...
        if (snapshot.size < 50 || progress === 0) break;
      }
    } finally {
      syncingPostprocesosVenta = false;
    }
    if (completas || advertencias || fallidas) {
      console.info(`[C9.39] Postprocesos: ${completas} completados · ${advertencias} con advertencia · ${fallidas} transitorios.`);
    }
    return { completas, advertencias, fallidas };
  }
  ...
  async function executeNewSaleAtomicCore(item, sale, docId, saleRef, user) {
    ...
    if (item.sessionId && item.sessionId !== sessionId) {
      const error = new Error('Esta venta pendiente pertenece a otra sesión POS y no se sincronizará automáticamente.');
      error.code = 'stale-pos-session';
      throw error;
    }
    const saleDayKey = cleanText(immutableSaleDayKey(sale, item.creadoEn), 20);
    if (!validBusinessDayKey(saleDayKey)) throw new Error('La venta pendiente no conserva un diaClave verificable; queda bloqueada para revisión y no afectará la jornada actual.');
    ...
    const activeJornadaId = cleanText(prepared.jornadaId || '', 120);
    const activeJornadaInicioISO = cleanText(prepared.jornadaInicioISO || '', 50);
    if (!activeJornadaId) throw new Error('No existe una jornada activa válida para numerar la venta.');

    // C9.39: una pendiente antigua jamás se adopta silenciosamente por una
    // apertura nueva. Esto evita que cuatro ventas olvidadas en localStorage
    // consuman #1..#4 y hagan que la primera venta real de hoy salga como #5.
    const pendingJornadaId = cleanText(sale.jornadaId || '', 120);
    if (pendingJornadaId && pendingJornadaId !== activeJornadaId) {
      const error = new Error('La venta pendiente pertenece a otra jornada y queda bloqueada para revisión; no consumirá un consecutivo de la jornada actual.');
      error.code = 'pending-other-jornada';
      throw error;
    }
    const pendingCreatedAt = timestampToDate(sale.fechaNegocioISO || sale.fechaISO || sale.fechaOficialISO || item.creadoEn);
    const activeOpening = timestampToDate(activeJornadaInicioISO);
    if (!pendingCreatedAt) {
      const error = new Error('La venta pendiente no conserva una fecha de creación verificable; queda bloqueada y no afectará el contador actual.');
      error.code = 'sale-day-missing';
      throw error;
    }
    if (activeOpening && pendingCreatedAt.getTime() < activeOpening.getTime()) {
      const error = new Error('La venta pendiente fue creada antes de la apertura de esta jornada; queda bloqueada para revisión y no consumirá comandas nuevas.');
      error.code = 'pending-before-jornada';
      throw error;
    }
    ...
    await firestoreDb.runTransaction(async transaction => {
      ...
      const controlRef = firestoreDb.collection('controlCaja').doc(firestoreDocId(saleDayKey));
      const controlSnap = await transaction.get(controlRef);
      const controlData = controlSnap.exists ? (controlSnap.data() || {}) : {};
      const controlOpeningDate = timestampToDate(controlData.jornadaInicioISO || controlData.aperturaHora);
      const controlOpeningISO = controlOpeningDate ? controlOpeningDate.toISOString() : '';
      const controlJornadaId = cleanText(controlData.jornadaId || buildJornadaId(saleDayKey, controlOpeningISO), 120);
      if (!controlOpeningISO || controlJornadaId !== activeJornadaId) {
        const error = new Error('La jornada activa cambió antes de confirmar la venta. No se emitió ningún número.');
        error.code = 'jornada-changed';
        throw error;
      }
      ...
      const counterJornadaId = cleanText(counters?.jornadaId || '', 120);
      if (!atomicCounter || counterJornadaId !== activeJornadaId) {
        for (const key of Array.from(atomicCounterPreparation.keys())) {
          if (String(key).startsWith(`${saleDayKey}|`)) atomicCounterPreparation.delete(key);
        }
        const error = new Error('El contador no corresponde a la jornada activa. Se reintentará sin emitir número.');
      ...
      const payload = clonePlain({
        ...
        jornadaInicioISO: activeJornadaInicioISO,
        ...
        consecutivoOrigen: 'firestore_transaccion_jornada_v5',
      ...
      // C9.39: esta transacción crítica contiene SOLO venta + contador. Ninguna
    ...
    try {
      const postResult = await postProcessConfirmedSale(resultSale, item.operacionId, item.ajustesInventario || {});
      resultSale.postProcesoEstado = 'completo';
      resultSale.postProcesoError = cleanText((postResult?.warnings || []).join(' | '), 450);
    } catch (error) {
      resultSale.postProcesoEstado = 'pendiente';
      resultSale.postProcesoError = cleanText(error?.message || error, 450);
      console.warn(`[C9.39] Venta #${resultSale.numeroVentaDia || '?'} confirmada; postproceso se reintentará:`, error);
    }
  ...
  async function executeSaleOperation(item) {
    ...
    const currentSessionId = requireExclusiveSessionId();
    if (item.sessionId && item.sessionId !== currentSessionId) {
      const error = new Error('La operación pertenece a una sesión POS anterior. Se mantiene bloqueada para evitar duplicaciones entre conexiones.');
      error.code = 'stale-pos-session';
      throw error;
    }
    ...
    let resultSale = { ...sale, _docId: docId };
    let alreadyApplied = false;
    // C9.39: el día de negocio se fija al crear la venta y nunca se vuelve a
    ...
    const saleDayKey = cleanText(immutableSaleDayKey(sale, item.creadoEn), 20);
    if (!validBusinessDayKey(saleDayKey)) throw new Error('La operación pendiente no conserva un diaClave verificable; no se reasignará automáticamente al día actual.');
    await firestoreDb.runTransaction(async transaction => {
      const saleSnap = await transaction.get(saleRef);
      ...
      if (item.tipo === 'delete') {
        ...
        const payload = clonePlain({
          ...
          _ultimaOperacionId: item.operacionId,
          schemaVersion: remote ? Math.max(1, Number(remote.schemaVersion || 1)) : 8,
          dispositivoId: item.deviceId,
          sessionId: remote?.sessionId || currentSessionId,
          diaClave: saleDayKey,
          mesClave: sale.mesClave || saleDayKey.slice(0, 7),
      ...
      for (const original of [...queue]) {
        ...
        try {
          ...
          const detailedMessage = permissionDenied
            ? `${message} Cuenta autenticada: ${firebaseAuth?.currentUser?.email || 'sin correo'}. Verifica que firestore.rules C9.39 esté publicado.`
            : message;
          queue = queue.map(item => item.operacionId === original.operacionId
          ...
          // C9.39: pendientes heredadas de otra sesión/jornada o sin fecha
          // verificable se aíslan. No consumen consecutivo y tampoco bloquean
          // las ventas válidas de la jornada actual.
          const codigoError = String(error?.code || '');
          const diaOriginal = immutableSaleDayKey(original.venta || {}, original.creadoEn || '');
          const bloqueoHeredado = ['pending-before-jornada', 'pending-other-jornada', 'sale-day-missing', 'stale-pos-session'].includes(codigoError)
            || (validBusinessDayKey(diaOriginal) && diaOriginal !== colombiaDayKey(new Date()) && /apertura|jornada/i.test(message));
          if (bloqueoHeredado) {
            queue = queue.map(item => item.operacionId === original.operacionId
              ? { ...item, estado: 'bloqueada', bloqueoPermanente: true, codigoError, ultimoError: detailedMessage }
              : item);
            writeSalesQueue(queue);
            blocked += 1;
            continue;
          ...
          // Las ventas NUEVAS válidas de la misma jornada sí se sincronizan en
          // FIFO estricto. Si la primera falla de forma transitoria, no dejamos
          // que una posterior tome su consecutivo.
  ...
  guardarVentaEnFirebase = async function(sale, docId = null) {
    ...
    const operation = {
      ...
      deviceId: getDeviceId(),
      sessionId: getExclusiveSessionId()
    };
  ...
  function updateQueueStatus() {
    const salesQueue = readSalesQueue();
    const blockedSales = salesQueue.filter(item => item?.bloqueoPermanente).length;
    const sales = Math.max(0, salesQueue.length - blockedSales);
    ...
    if (element) {
      if (total) {
        element.textContent = `${total} operación(es) pendiente(s) de sincronizar${blockedSales ? ` · ${blockedSales} venta(s) antigua(s) bloqueada(s) para revisión` : ''}`;
        element.className = 'mt-1 text-xs font-semibold text-amber-700';
      } else if (blockedSales) {
        element.textContent = `Sin pendientes activos · ${blockedSales} venta(s) antigua(s) bloqueada(s) para revisión`;
        element.className = 'mt-1 text-xs font-semibold text-slate-600';
      } else {
        element.textContent = 'Sin operaciones pendientes';
        element.className = 'mt-1 text-xs font-semibold text-green-700';
      }
    }
  }

  window.diagnosticarColaVentasC939 = function() {
    ...
    const rows = queue.map(item => ({
      ...
      diaClave: immutableSaleDayKey(item.venta || {}, item.creadoEn || ''),
      fechaISO: item.venta?.fechaISO || '',
      comanda: item.venta?.comanda || 0,
      numeroVentaDia: item.venta?.numeroVentaDia || 0,
  ...
  function initializeHardening() {
    ...
    setInterval(() => {
      updateQueueStatus();
      // C9.39: una venta que falló por un corte breve se reintenta aunque el
      ...
      if (isOnlineFirebase() && readSalesQueue().length && !syncingSalesSecurely) {
        sincronizarVentasPendientesEnSegundoPlano().catch(error => {
          console.warn('[C9.39] Reintento automático de ventas pendiente:', error);
        });
      }
      if (isOnlineFirebase() && !syncingPostprocesosVenta) {
        syncPendingSalePostprocesses().catch(error => {
          console.warn('[C9.39] Reintento de postprocesos de venta:', error);
        });
      }
    }, 15000);
  }
...
console.info('[Señor Arepa] Código activo: C9.39 · proyecto esperado: prsenorarepa');
