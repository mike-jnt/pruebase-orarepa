(() => {
  ...
  const HARDENING_VERSION = '2026.08.03-C9.3';
  ...
  async function diagnosticarPermisosFirestoreC93(mostrarAviso = false) {
    ...
    const pruebas = [
      ...
      ['controlCajaLectura', () => firestoreDb.collection('controlCaja').doc('__diagnostico_c93__').get()],
      ['movimientosInventarioGet', () => firestoreDb.collection('movimientosInventario').doc('__diagnostico_c93__').get()]
    ];
    ...
    console.info('[Señor Arepa] Diagnóstico Firestore C9.3', resultado);
    if (mostrarAviso) {
      notify(
        correcto
          ? `Permisos Firestore C9.3 verificados para ${email}.`
          : `Las reglas C9.3 no están activas para ${email || 'la cuenta actual'}. Publica firestore.rules antes de continuar.`,
        correcto ? 'success' : 'error'
      );
    }
    return resultado;
  }
  window.diagnosticarPermisosFirestoreC93 = diagnosticarPermisosFirestoreC93;
  ...
  aplicarAjustesInventarioLocal = function(adjustments = {}) {
    ...
    try {
      ...
      if (changed) {
        localStorage.setItem(INVENTARIO_STORAGE_KEY, JSON.stringify(inventory));
  ...
  function writeSalesQueue(items = []) {
    ...
    localStorage.setItem(SALES_QUEUE_KEY, serialized);
    updateQueueStatus();
    return list;
  }
      ...
      for (const original of [...queue]) {
        ...
        try {
          ...
          const detailedMessage = permissionDenied
            ? `${message} Cuenta autenticada: ${firebaseAuth?.currentUser?.email || 'sin correo'}. Publica firestore.rules C9.3: esta versión habilita la lectura GET de movimientosInventario que la transacción usa para impedir descuentos duplicados.`
            : message;
          queue = queue.map(item => item.operacionId === original.operacionId
  ...
  function writeCashQueue(items) {
    ...
    localStorage.setItem(CASH_QUEUE_KEY, JSON.stringify(list));
    updateQueueStatus();
  }
  ...
  function migrateLocalCatalogAndSales() {
    try {
      ...
      if (changed) localStorage.setItem(INVENTARIO_STORAGE_KEY, JSON.stringify(inventory));
      inventarioBebidasEstado = obtenerMapaInventarioDesdeArray(inventory);
    } catch (error) {
  ...
  function initializeHardening() {
    ...
    setTimeout(() => {
      if (isOnlineFirebase()) diagnosticarPermisosFirestoreC93(false);
    }, 1200);
    setInterval(() => {
      updateQueueStatus();
