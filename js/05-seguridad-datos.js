(() => {
  ...
  const HARDENING_VERSION = '2026.08.02-C8.1';
  ...
  async function executeSaleOperation(item) {
    ...
    await firestoreDb.runTransaction(async transaction => {
      ...
      const targetVersion = item.tipo === 'delete'
        ? remoteVersion
        : Math.max(remoteVersion + 1, Number(sale.version || remoteVersion + 1));
      ...
      if (item.tipo === 'delete') {
        ...
        transaction.set(saleRef, payload, { merge: true });
    ...
    try {
      for (const original of [...queue]) {
        ...
        try {
          ...
          queue = queue.map(item => item.operacionId === original.operacionId
            ? { ...item, intentos: Number(item.intentos || 0) + 1, ultimoError: message, estado: 'error' }
            : item);
          blockedSales.add(identity);
          if (original.tipo !== 'delete' && typeof upsertVentaEnCacheLocal === 'function') {
            upsertVentaEnCacheLocal({ ...original.venta, _syncEstado: 'error', syncError: message });
          }
          writeSalesQueue(queue);
          failed += 1;
  ...
  guardarVentaEnFirebase = async function(sale, docId = null) {
    ...
    const normalized = secureNormalizarVenta(sale || {});
    ...
    const operation = {
      ...
      ultimoError: '',
      estado: 'pendiente',
      versionEsperada: Math.max(0, normalized.version - 1),
      deviceId: getDeviceId()
    };
  ...
  async function executeCashOperation(item) {
    ...
    await firestoreDb.runTransaction(async transaction => {
      ...
      const nextVersion = Math.max(remoteVersion + 1, Number(payload.version || remoteVersion + 1));
      const timestamp = fieldValueServerTimestamp();
