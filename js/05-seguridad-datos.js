(() => {
  ...
  const HARDENING_VERSION = '2026.08.03-C9.2';
      ...
      for (const original of [...queue]) {
        ...
        try {
          ...
          const detailedMessage = permissionDenied
            ? `${message} Cuenta autenticada: ${firebaseAuth?.currentUser?.email || 'sin correo'}. Verifica que firestore.rules actualizado esté publicado.`
            : message;
          queue = queue.map(item => item.operacionId === original.operacionId
