(() => {
  ...
  const HARDENING_VERSION = '2026.08.06-C9.27';
  ...
  async function diagnosticarPermisosFirestoreC94(mostrarAviso = false) {
    ...
    console.info('[Señor Arepa] Diagnóstico Firestore C9.27', resultado);
    if (mostrarAviso) {
      notify(
        correcto
          ? `Permisos Firestore C9.27 verificados para ${email}.`
          : `Las reglas C9.27 no están activas para ${email || 'la cuenta actual'}. Publica firestore.rules antes de continuar.`,
      ...
      for (const original of [...queue]) {
        ...
        try {
          ...
          const detailedMessage = permissionDenied
            ? `${message} Cuenta autenticada: ${firebaseAuth?.currentUser?.email || 'sin correo'}. Publica firestore.rules C9.27: esta versión habilita la lectura GET de movimientosInventario que la transacción usa para impedir descuentos duplicados.`
            : message;
          queue = queue.map(item => item.operacionId === original.operacionId
...
console.info('[Señor Arepa] Código activo: C9.27 · proyecto esperado: prsenorarepa');
