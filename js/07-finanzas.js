(() => {
  ...
  async function cargarPaginaMovimientos(reset = false) {
    ...
    try {
      ...
      console.error('[C9.21] Error cargando movimientos:', error);
  ...
  async function cargarAnaliticaCompleta() {
    ...
    try {
      ...
      console.error('[C9.21] No se pudo cargar la analítica completa:', error);
  ...
  function resolverValoresCierreFinanzas(control = {}, resumenDiario = null) {
    ...
    const domicilios = primerNumeroValido(
      resumen.ajusteDomiciliosTransferencia, resumen.ajusteDomiciliosDigitales, resumen.domiciliosDescontados,
      control.ajusteDomiciliosTransferencia, control.domiciliosDescontados,
      diario.domiciliosTransferenciaValor, diario.ajusteDomiciliosTransferencia, diario.domiciliosDescontados
    );
    const efectivoNeto = primerNumeroValido(
      ...
      Math.max(0, efectivo - domicilios)
    );
    const cierreOperativo = primerNumeroValido(
      ...
      Math.max(0, efectivoNeto + transferencias),
  ...
  async function obtenerResumenDiarioFinanzas(diaClave) {
    try {
      ...
      console.warn(`[C9.21] No se pudo consultar resumenVentasDiario/${diaClave}:`, error);
      return null;
    }
  }
  ...
  async function sincronizarCierresCajaEnFinanzas(forzarRecarga = true, silencioso = false) {
    ...
    try {
      ...
      console.error('[C9.21] No se pudieron sincronizar los cierres:', error);
  ...
  async function sincronizarPagosNominaEnFinanzas(forzarRecarga = true, silencioso = false) {
    ...
    try {
      ...
      console.error('[C9.21] No se pudo sincronizar la nómina:', error);
