(() => {
  ...
  inicializarFirebaseVentas = async function() {
    ...
    try {
      ...
      // C9.27 usa su propia cola local. Se desactiva la persistencia IndexedDB
      // de Firestore para evitar escrituras antiguas atascadas y conflictos entre pestañas.
      saPersistenciaIntentada = true;
      console.info('[Firebase C9.27] Caché persistente del SDK desactivada; cola local propia activa.');
      firebaseAuth.onAuthStateChanged(async user => {
        ...
        if (!diagnosticoConexion.rest) {
          ...
          console.error('[Firebase C9.27] Diagnóstico de conexión fallido:', diagnosticoConexion);
          saActualizarEstadoCola();
          return;
        }
        ...
        if (!diagnosticoConexion.sdk) {
          ...
          console.error('[Firebase C9.27] REST responde, pero WebChannel no. Abre el sistema desde localhost y desactiva extensiones para probar.', diagnosticoConexion);
          saActualizarEstadoCola();
          return;
        }
  ...
  async function saReservarConsecutivos(diaClave, requiereDomicilio) {
    // C9.27: el número definitivo se asigna dentro de la misma transacción
    // que crea la venta en Firestore (05-seguridad-datos.js). Así, una venta
    // que falle antes de guardarse no consume un número y no deja saltos.
    const provisional = typeof obtenerSiguienteNumeracionDelDia === 'function'
      ? obtenerSiguienteNumeracionDelDia(diaClave)
      : { comanda: saConsecutivoTemporal(diaClave, 'T'), recibo: saConsecutivoTemporal(diaClave, 'R') };
    const domicilioProvisional = requiereDomicilio
      ? (typeof obtenerSiguienteNumeroDomicilioDelDia === 'function'
          ? obtenerSiguienteNumeroDomicilioDelDia(diaClave)
          : saConsecutivoTemporal(diaClave, 'D'))
      : null;
    return {
      comanda: provisional.comanda,
      recibo: provisional.recibo,
      domicilio: domicilioProvisional,
      temporal: true
    };
  }
