(() => {
  ...
  inicializarFirebaseVentas = async function() {
    ...
    try {
      ...
      // C9.8 usa su propia cola local. Se desactiva la persistencia IndexedDB
      // de Firestore para evitar escrituras antiguas atascadas y conflictos entre pestañas.
      saPersistenciaIntentada = true;
      console.info('[Firebase C9.8] Caché persistente del SDK desactivada; cola local propia activa.');
      firebaseAuth.onAuthStateChanged(async user => {
        ...
        if (!diagnosticoConexion.rest) {
          ...
          console.error('[Firebase C9.8] Diagnóstico de conexión fallido:', diagnosticoConexion);
          saActualizarEstadoCola();
          return;
        }
        ...
        if (!diagnosticoConexion.sdk) {
          ...
          console.error('[Firebase C9.8] REST responde, pero WebChannel no. Abre el sistema desde localhost y desactiva extensiones para probar.', diagnosticoConexion);
          saActualizarEstadoCola();
          return;
        }
