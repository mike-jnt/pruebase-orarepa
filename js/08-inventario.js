(() => {
  ...
  async function cargarPaginaInventario(reset = false) {
    ...
    try {
      ...
      console.info(`[Inventario C9.39] Página ${paginaActual + 1} cargada`, {
        visibles: inventario.length,
        hayPaginaSiguiente
      });
    } catch (error) {
      console.error('[Inventario C9.39] No se pudo cargar la página:', error);
  ...
  async function cambiarPagina(delta) {
    ...
    if (direccion > 0 && !cursoresPagina[destino]) {
      console.warn('[Inventario C9.39] No existe cursor para la página solicitada.');
      return;
    }
  ...
  async function guardarEdicion(index) {
    ...
    const bebidaVinculadaMenu = item.categoria === 'bebida' && Boolean(item.productId || item.inventoryId);
    if (bebidaVinculadaMenu && normalizarClaveInventario(nuevoNombre) !== normalizarClaveInventario(item.nombre)) {
      alert('Esta bebida está vinculada al menú. Para cambiar su nombre usa “Editar menú”; al guardar, C9.39 actualizará automáticamente este mismo registro de inventario sin perder el stock.');
      if ($(`editar-nombre-${index}`)) $(`editar-nombre-${index}`).value = item.nombre;
      return;
    }
    try {
