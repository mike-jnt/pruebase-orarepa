(() => {
  ...
  async function cargarPaginaInventario(reset = false) {
    ...
    try {
      ...
      console.info(`[Inventario C9.27] Página ${paginaActual + 1} cargada`, {
        visibles: inventario.length,
        hayPaginaSiguiente
      });
    } catch (error) {
      console.error('[Inventario C9.27] No se pudo cargar la página:', error);
  ...
  async function cambiarPagina(delta) {
    ...
    if (direccion > 0 && !cursoresPagina[destino]) {
      console.warn('[Inventario C9.27] No existe cursor para la página solicitada.');
      return;
    }
