(() => {
  ...
  function crearPaginacion() {
    const tablaEmp = $('tabla-empleados')?.closest('.nomina-table-wrap, .overflow-x-auto');
    if (tablaEmp && !$('paginacionEmpleados')) {
      tablaEmp.insertAdjacentHTML('afterend', `
        <div id="paginacionEmpleados" class="nomina-pagination">
          <span id="infoPaginacionEmpleados" class="text-sm text-gray-500">Página 1 · máximo ${EMPLEADOS_PAGE_SIZE} empleados</span>
          <div class="nomina-pagination-controls"><button id="btnEmpAnterior" type="button" class="nomina-page-button">← Anterior</button><button id="btnEmpSiguiente" type="button" class="nomina-page-button">Siguiente →</button></div>
        </div>`);
    ...
    const tablaPagos = $('tabla-historial')?.closest('.nomina-table-wrap, .overflow-x-auto');
    if (tablaPagos && !$('paginacionPagos')) {
      tablaPagos.insertAdjacentHTML('afterend', `
        <div id="paginacionPagos" class="nomina-pagination">
          <span id="infoPaginacionPagos" class="text-sm text-gray-500">Página 1 · máximo ${PAGOS_PAGE_SIZE} pagos</span>
          <div class="nomina-pagination-controls"><button id="btnPagoAnterior" type="button" class="nomina-page-button">← Anterior</button><button id="btnPagoSiguiente" type="button" class="nomina-page-button">Siguiente →</button></div>
        </div>`);
      $('btnPagoAnterior').addEventListener('click', () => cambiarPaginaPagos(-1));
      $('btnPagoSiguiente').addEventListener('click', () => cambiarPaginaPagos(1));
    }
    const acciones = document.querySelector('.nomina-toolbar');
    if (acciones && !$('btnMigrarNomina')) {
      acciones.insertAdjacentHTML('afterbegin', '<button id="btnMigrarNomina" type="button" class="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2">☁️ Subir datos locales</button>');
  ...
  function mostrarEmpleados() {
    ...
    tabla.innerHTML = empleados.map((emp) => {
      ...
      return `<tr>
        ...
        <td>${escapeHtml(formatearCOP(emp.valorDia))}</td>
        <td>${emp.diasTrabajados}</td>
        <td>${diasFaltados}</td>
        <td>${escapeHtml(generarRangoQuincena(emp.fechaPago))}</td>
        <td>
          <div>Total base: <strong>${escapeHtml(formatearCOP(totalTrabajado))}</strong></div>
          ${extras > 0 ? `<div class="text-xs text-blue-600">Horas extra: +${escapeHtml(formatearCOP(extras))}</div>` : ''}
          ${emp.bonificacion > 0 ? `<div class="text-xs text-blue-600">Bonificación: +${escapeHtml(formatearCOP(emp.bonificacion))}</div>` : ''}
          ${emp.prestamo > 0 ? `<div class="text-xs text-red-600">Préstamo: -${escapeHtml(formatearCOP(emp.prestamo))}</div>` : ''}
          <div class="text-sm text-green-700 font-bold">Total a pagar: ${escapeHtml(formatearCOP(total))}</div>
          ${diasFaltados > 0 ? `<div class="text-xs text-red-600">Faltas: -${escapeHtml(formatearCOP(diferencia))}</div>` : ''}
        </td>
        <td><span class="nomina-status ${emp.estadoPago === 'Pagado' ? 'pagado' : 'pendiente'}">${escapeHtml(emp.estadoPago)}</span></td>
        <td><div class="nomina-actions">
          <button onclick="registrarAsistencia('${escapeHtml(emp.id)}')" class="bg-blue-100 text-blue-800" ${emp.estadoPago === 'Pagado' ? 'disabled' : ''}>Asistencia</button>
          ${emp.estadoPago === 'Pendiente'
            ? `<button onclick="marcarComoPagado('${escapeHtml(emp.id)}')" class="bg-green-100 text-green-800">Marcar pagado</button>`
            : `<button onclick="desmarcarPagado('${escapeHtml(emp.id)}')" class="bg-yellow-100 text-yellow-800">Desmarcar</button>`}
          <button onclick="generarPDF('${escapeHtml(emp.id)}')" class="bg-purple-100 text-purple-800">Generar PDF</button>
          <button onclick="eliminarEmpleado('${escapeHtml(emp.id)}')" class="bg-red-100 text-red-800">Eliminar</button>
        </div></td>
      </tr>`;
    }).join('');
  }
  ...
  function mostrarHistorial() {
    ...
    if (!tbody) return;
    tbody.innerHTML = historial.map((p) => `<tr>
      <td>${escapeHtml(p.nombre)}</td><td>${escapeHtml(p.cargo)}</td><td>${escapeHtml(formatearFecha(p.fechaPago))}</td>
      <td>${escapeHtml(p.quincena)}</td><td>${p.diasTrabajados}</td><td>${escapeHtml(formatearCOP(p.valorDia))}</td>
      <td>${p.horasExtras}</td><td>${escapeHtml(formatearCOP(p.bonificacion))}</td><td>${escapeHtml(formatearCOP(p.totalPagado))}</td>
    </tr>`).join('') || '<tr><td colspan="9" class="p-4 text-center text-gray-500">No hay pagos en esta página.</td></tr>';
  }
