...
function guardarCatalogoProductosLocal() {
  try {
    localStorage.setItem(CATALOGO_PRODUCTOS_STORAGE_KEY, JSON.stringify(catalogoProductos));
  } catch (error) {
...
function desactivarFirebaseCajaPorPermisos(error) {
  ...
  if (!controlCajaFirebasePermisosAvisados) {
    ...
    console.warn(
      `Control de caja funcionando temporalmente en modo local. Firestore rechazó controlCaja para ${emailSesion} en ${projectIdSesion}. Publica firestore.rules C9.4.`,
      error
    );
    if (typeof window.notificarSistema === 'function') {
      window.notificarSistema(`Firestore rechazó controlCaja para ${emailSesion}. Publica las reglas C9.4.`, 'error');
    }
  }
