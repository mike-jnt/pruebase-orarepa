...
const SENOR_AREPA_STORAGE_VERSION = 'C9.27';
...
function leerLocalStorageJSONSeguro(key, fallback) {
  try {
    ...
    console.warn(`[Storage C9.27] Se descartó una copia local dañada: ${key}.`, error);
...
function guardarLocalStorageSeguro(key, value, opciones = {}) {
  ...
  try {
    ...
    console.error(`[Storage C9.27] No se pudo serializar ${key}:`, error);
    return false;
  }
  try {
    ...
    if (!esErrorCuotaStorage(error)) {
      console.error(`[Storage C9.27] No se pudo guardar ${key}:`, error);
      return false;
    }
    ...
    try {
      localStorage.setItem(key, texto);
      console.warn(
        `[Storage C9.27] Se liberó espacio automáticamente para ${key}.`,
      ...
      if (critico) {
        console.error(`[Storage C9.27] ${mensaje}`, segundoError);
        try {
        ...
        console.warn(`[Storage C9.27] ${mensaje}`, segundoError);
      }
      return false;
    }
  }
...
function migrarAlmacenamientoLocalC95() {
  try {
    ...
    console.info('[Señor Arepa C9.27] Almacenamiento local optimizado.', estimarUsoLocalStorage());
  } catch (error) {
    console.warn('[Señor Arepa C9.27] No se pudo completar la migración de almacenamiento:', error);
  }
}
...
window.limpiarCacheLocalSenorArepa = function limpiarCacheLocalSenorArepa() {
  ...
  try { localStorage.removeItem(ULTIMA_VENTA_GUARDADA_KEY); } catch (_) {}
  console.info('[Señor Arepa C9.27] Limpieza manual terminada.', estimarUsoLocalStorage());
...
function desactivarFirebaseCajaPorPermisos(error) {
  if (!firebaseAuth?.currentUser) {
    console.info('[Firebase C9.27] Se omitió una consulta de controlCaja porque la sesión todavía no estaba autenticada.');
    return;
  }
  ...
  if (!controlCajaFirebasePermisosAvisados) {
    ...
    console.warn(
      `Control de caja funcionando temporalmente en modo local. Firestore rechazó controlCaja para ${emailSesion} en ${projectIdSesion}. Publica firestore.rules C9.27.`,
      error
    );
    if (typeof window.notificarSistema === 'function') {
      window.notificarSistema(`Firestore rechazó controlCaja para ${emailSesion}. Publica las reglas C9.27.`, 'error');
    }
  }
...
function programarMigracionAlmacenamientoC98() {
  const ejecutar = () => {
    try {
      ...
      console.warn('[Señor Arepa C9.27] La migración local se omitió sin bloquear el sistema:', error);
    }
  };
...
function configurarFirestoreRedRobusta(db) {
  ...
  try {
    ...
    console.info('[Firebase C9.27] Transporte robusto activado (long polling).');
  } catch (error) {
    const mensaje = String(error?.message || error || '');
    if (/already been started|settings can no longer be changed|failed-precondition/i.test(mensaje)) {
      console.warn('[Firebase C9.27] Firestore ya estaba iniciado; se conservaron sus ajustes actuales.');
    } else {
      console.warn('[Firebase C9.27] No fue posible aplicar los ajustes de red:', error);
    }
  }
  return db;
}
...
async function reconectarFirestoreSeguro(motivo = 'manual') {
  ...
  try {
    ...
    registrarHeartbeatFirebase();
    console.info(`[Firebase C9.27] Conexión restablecida (${motivo}).`);
    return true;
  } catch (error) {
    console.warn(`[Firebase C9.27] No se pudo restablecer la conexión (${motivo}):`, error);
    return false;
  } finally {
...
async function diagnosticarConexionFirebaseC98(opciones = {}) {
  ...
  if (!silencioso) {
    ...
    if (resultado.rest && resultado.sdk) {
      console.info('[Firebase C9.27] Conexión completa verificada.',
      ...
      console.error('[Firebase C9.27] Firebase responde por REST, pero el canal del SDK está bloqueado por red, antivirus, proxy o extensión.', resultado);
    } else {
      console.error('[Firebase C9.27] No se pudo verificar Firestore.', resultado);
    }
  }
  return resultado;
}
...
function cancelarPedidoActual() {
    ...
    return `
      <tr class="${ventaCancelada ? 'sa-sales-row-cancelled' : ''}">
        <td class="sa-sales-select-column" style="display:${esAdmin() ? 'table-cell' : 'none'}">
          <input type="checkbox" ${ventaSeleccionada ? 'checked' : ''} onchange="toggleSeleccionVenta('${ventaKey}', this.checked)" class="w-4 h-4 accent-red-700 cursor-pointer" aria-label="Seleccionar venta ${v.comanda ?? v.recibo ?? (i + 1)}" />
        </td>
        <td class="sa-sales-number-cell">${v.comanda ?? v.recibo ?? (i + 1)}</td>
        <td class="sa-sales-date-cell">${formatearFechaHoraColombia(v.fechaISO || v.fecha)}</td>
      ...
      <article class="sa-sales-mobile-card ${ventaCancelada ? 'sa-sales-mobile-cancelled' : ''}">
        <div class="sa-sales-mobile-card-header">
          <div>
            ...
            <p class="text-xs font-bold text-amber-700">Venta #${v.comanda ?? v.recibo ?? (i + 1)}</p>
  ...
  function obtenerNumeroDiarioVenta(venta = {}) {
    ...
    if (Number.isFinite(comanda) && comanda > 0) return comanda;
    const recibo = Number(venta?.recibo || 0);
    return Number.isFinite(recibo) && recibo > 0 ? recibo : 0;
  }
  ...
  function obtenerVentasDelDiaActualConIndices() {
    ...
    return ventas
      ...
      .sort((a, b) => {
        // El consecutivo oficial diario manda. Los registros temporales quedan
        // ordenados por fecha hasta que Firestore les asigne el número final.
        const numeroA = obtenerNumeroDiarioVenta(a.venta);
        const numeroB = obtenerNumeroDiarioVenta(b.venta);
        if (numeroA !== numeroB) return numeroB - numeroA;
        return String(b.venta.fechaISO || b.venta.fecha || '').localeCompare(String(a.venta.fechaISO || a.venta.fecha || ''));
      });
  }
