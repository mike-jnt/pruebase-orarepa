// C9.39: este proyecto ya no instala Service Worker. En localhost se eliminan
// registros/cachés heredados para evitar errores "Failed to fetch" de sw.js
// provenientes de versiones antiguas.
(function limpiarServiceWorkerLocalHeredado() {
  try {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.getRegistrations()
        ...
        .then(resultados => {
          if (resultados.some(Boolean)) console.info('[C9.39] Service Worker local heredado eliminado. Recarga una vez si estaba activo.');
        })
        .catch(() => {});
    }
...
const usuariosSistema = {
  ...
  alfredo: { email: "alfredo@local.io", rol: "cajero", nombre: "Alfredo" },
  cajero1: { email: "cajero1@local.io", rol: "cajero", nombre: "Cajero 2" }
};
...
const SENOR_AREPA_STORAGE_VERSION = 'C9.39';
...
function leerLocalStorageJSONSeguro(key, fallback) {
  try {
    ...
    console.warn(`[Storage C9.39] Se descartó una copia local dañada: ${key}.`, error);
...
function guardarLocalStorageSeguro(key, value, opciones = {}) {
  ...
  try {
    ...
    console.error(`[Storage C9.39] No se pudo serializar ${key}:`, error);
    return false;
  }
  try {
    ...
    if (!esErrorCuotaStorage(error)) {
      console.error(`[Storage C9.39] No se pudo guardar ${key}:`, error);
      return false;
    }
    ...
    try {
      localStorage.setItem(key, texto);
      console.warn(
        `[Storage C9.39] Se liberó espacio automáticamente para ${key}.`,
      ...
      if (critico) {
        console.error(`[Storage C9.39] ${mensaje}`, segundoError);
        try {
        ...
        console.warn(`[Storage C9.39] ${mensaje}`, segundoError);
      }
      return false;
    }
  }
...
function compactarReferenciaVenta(venta = {}) {
  ...
  return {
    ...
    numeroVentaDia: Number(normalizada.numeroVentaDia || 0),
...
function migrarAlmacenamientoLocalC95() {
  try {
    ...
    console.info('[Señor Arepa C9.39] Almacenamiento local optimizado.', estimarUsoLocalStorage());
  } catch (error) {
    console.warn('[Señor Arepa C9.39] No se pudo completar la migración de almacenamiento:', error);
  }
}
...
window.limpiarCacheLocalSenorArepa = function limpiarCacheLocalSenorArepa() {
  ...
  try { localStorage.removeItem(ULTIMA_VENTA_GUARDADA_KEY); } catch (_) {}
  console.info('[Señor Arepa C9.39] Limpieza manual terminada.', estimarUsoLocalStorage());
...
function obtenerResumenControlCajaGuardado(control = {}) {
  const candidatos = [
    control.resumenCierre,
    control.resumenCaja,
    control.cierreResumen,
    control.resumen
  ];
  return candidatos.find(valor => valor && typeof valor === 'object' && !Array.isArray(valor)) || {};
}

function normalizarControlCaja(control = {}, diaClave = "") {
  const resumenGuardado = obtenerResumenControlCajaGuardado(control);
  const aperturaMonto = obtenerNumeroCaja(control.aperturaMonto, resumenGuardado.aperturaMonto);
  const cierreMonto = obtenerNumeroCaja(control.cierreMonto, resumenGuardado.montoContado, resumenGuardado.cierreMonto);
  const totalEfectivo = obtenerNumeroCaja(
    resumenGuardado.totalEfectivo,
    resumenGuardado.efectivoBrutoSistema,
    control.totalEfectivo,
    control.efectivoBrutoSistema,
    control.efectivoSistema,
    control.ingresoEfectivo
  );
  const totalTransferencias = obtenerNumeroCaja(
    resumenGuardado.totalTransferencias,
    resumenGuardado.totalOtrosMedios,
    control.totalTransferencias,
    control.totalOtrosMedios,
    control.transferencias,
    control.transferenciasSistema,
    control.ingresoDigital
  );
  const ajusteDomiciliosTransferencia = obtenerNumeroCaja(
    resumenGuardado.ajusteDomiciliosTransferencia,
    resumenGuardado.ajusteDomiciliosDigitales,
    control.ajusteDomiciliosTransferencia,
    control.ajusteDomiciliosDigitales,
    control.domiciliosDescontados,
    control.domiciliosTransferencia
  );
  const totalDomicilios = obtenerNumeroCaja(
    resumenGuardado.totalDomicilios,
    resumenGuardado.domiciliosTotales,
    resumenGuardado.domiciliosDescontados,
    control.totalDomicilios,
    control.domiciliosTotales,
    control.domiciliosDescontados,
    Number(resumenGuardado.ajusteDomiciliosTransferencia || 0) + Number(resumenGuardado.ajusteDomiciliosEfectivo || 0),
    Number(control.ajusteDomiciliosTransferencia || 0) + Number(control.ajusteDomiciliosEfectivo || 0),
    ajusteDomiciliosTransferencia
  );
  const efectivoNetoSistema = obtenerNumeroCaja(
    resumenGuardado.efectivoNetoSistema,
    resumenGuardado.efectivoNeto,
    control.efectivoNetoSistema,
    control.efectivoNeto,
    redondearPago(totalEfectivo - totalDomicilios)
  );
  const totalSistemaReal = obtenerNumeroCaja(
    resumenGuardado.totalSistemaReal,
    resumenGuardado.cierreRealDia,
    control.totalSistemaReal,
    control.cierreRealDia,
    Math.max(0, efectivoNetoSistema + totalTransferencias)
  );
  const montoEsperadoCaja = obtenerNumeroCaja(
    resumenGuardado.montoEsperadoCaja,
    resumenGuardado.dineroEsperado,
    control.montoEsperadoCaja,
    control.dineroEsperado,
    control.cajaEsperada,
    aperturaMonto + efectivoNetoSistema
  );
  const diferencia = obtenerNumeroCaja(
    resumenGuardado.diferencia,
    control.diferencia,
    cierreMonto - montoEsperadoCaja
  );
  ...
  return {
    ...
    ajusteDomiciliosTransferencia,
    ajusteDomiciliosDigitales: ajusteDomiciliosTransferencia,
    ajusteDomiciliosEfectivo: obtenerNumeroCaja(
      resumenGuardado.ajusteDomiciliosEfectivo,
      control.ajusteDomiciliosEfectivo,
      Math.max(0, totalDomicilios - ajusteDomiciliosTransferencia)
    ),
    totalDomicilios,
    domiciliosTotales: totalDomicilios,
    domiciliosDescontados: totalDomicilios,
    efectivoNetoSistema,
    montoEsperadoCaja,
    dineroEsperado: montoEsperadoCaja,
    cajaEsperada: montoEsperadoCaja,
    totalSistemaReal,
    cierreRealDia: totalSistemaReal,
    diferencia,
    resumenCierre: Object.keys(resumenGuardado).length ? resumenGuardado : control.resumenCierre
  };
}
...
function tieneResumenGuardadoControl(control = {}) {
  const resumenGuardado = obtenerResumenControlCajaGuardado(control);
  const fuente = { ...control, ...resumenGuardado };
  const campos = [
    ...
    'efectivoNetoSistema', 'totalSistemaReal', 'montoEsperadoCaja', 'montoContado', 'diferencia'
  ];
  return campos.some(campo => Number(fuente?.[campo] || 0) !== 0);
}
...
function obtenerResumenCajaDiaParaControl(diaClave, control = {}) {
  control = normalizarControlCaja(control, diaClave);
  ...
  const totalDomicilios = primeroConValorNumerico(
    calculado.totalDomicilios,
    control.totalDomicilios,
    control.domiciliosTotales,
    control.domiciliosDescontados,
    Number(control.ajusteDomiciliosDigitales || 0) + Number(control.ajusteDomiciliosEfectivo || 0),
    ajusteDomiciliosTransferencia
  );
  const efectivoNetoSistema = primeroConValorNumerico(
    calculado.efectivoNetoSistema,
    control.efectivoNetoSistema,
    redondearPago(Number(control.montoEsperadoCaja || 0) - Number(control.aperturaMonto || 0)),
    redondearPago(totalEfectivo - totalDomicilios)
  );
  ...
  const totalDomiciliosGuardado = totalDomicilios;
  ...
  return {
    ...
    ajusteDomiciliosTransferencia,
    ajusteDomiciliosEfectivo: primeroConValorNumerico(calculado.ajusteDomiciliosEfectivo, control.ajusteDomiciliosEfectivo, Math.max(0, totalDomicilios - ajusteDomiciliosTransferencia)),
    totalDomicilios,
    domiciliosTotales: totalDomicilios,
    domiciliosDescontados: totalDomicilios,
    efectivoBrutoSistema: totalEfectivo,
...
function desactivarFirebaseCajaPorPermisos(error) {
  if (!firebaseAuth?.currentUser) {
    console.info('[Firebase C9.39] Se omitió una consulta de controlCaja porque la sesión todavía no estaba autenticada.');
    return;
  }
  ...
  if (!controlCajaFirebasePermisosAvisados) {
    ...
    console.warn(
      `Control de caja funcionando temporalmente en modo local. Firestore rechazó controlCaja para ${emailSesion} en ${projectIdSesion}. Publica firestore.rules C9.39.`,
      error
    );
    if (typeof window.notificarSistema === 'function') {
      window.notificarSistema(`Firestore rechazó controlCaja para ${emailSesion}. Publica las reglas C9.39.`, 'error');
    }
  }
...
function calcularResumenCajaDia(diaClave) {
  ...
  const totalDomicilios = redondearPago(
    ventasActivas.reduce((acc, venta) => acc + obtenerValorDomicilio(venta), 0)
  );
  ...
  const ajusteDomiciliosEfectivo = redondearPago(Math.max(0, totalDomicilios - ajusteDomiciliosTransferencia));

  // El valor completo de los domicilios no pertenece a la venta del negocio.
  // Se descuenta del efectivo operativo esperado, sin importar cómo pagó el cliente.
  const efectivoBrutoSistema = redondearPago(totalEfectivo);
  const efectivoNetoSistema = redondearPago(efectivoBrutoSistema - totalDomicilios);
  ...
  return {
    ...
    totalVentas,
    ventaRealNegocio: totalVentas,
    totalCobrado,
    totalCobradoClientes: totalCobrado,
    totalDomicilios,
    domiciliosTotales: totalDomicilios,
    domiciliosDescontados: totalDomicilios,
  ...
  contenedor.innerHTML = `
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-3">
      ...
      <div class="rounded-xl border border-red-100 bg-red-50 p-4">
        <p class="text-xs uppercase tracking-wide text-gray-500">Domicilios descontados</p>
        <p class="text-xl font-extrabold text-red-700 mt-1">${formatearCOP(resumen.totalDomicilios || resumen.domiciliosDescontados || 0)}</p>
        <p class="text-xs text-gray-500 mt-1">Se resta el valor total de los envíos del cierre operativo</p>
      </div>
    ...
    <div class="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700 space-y-1">
      ...
      <p><strong>Caja esperada:</strong> ${formatearCOP(esperado)} <span class="text-xs text-gray-500">(apertura + efectivo recibido - domicilios totales)</span></p>
      <p><strong>Efectivo neto esperado:</strong> ${formatearCOP(resumen.efectivoNetoSistema || 0)} <span class="text-xs text-gray-500">(efectivo recibido - domicilios totales)</span></p>
      <p><strong>Cierre real del día:</strong> ${formatearCOP(cierreRealDia)} <span class="text-xs text-gray-500">(efectivo + transferencias - domicilios totales)</span></p>
...
function renderContenidoModalDesgloseCierre(control = {}, resumen = {}) {
  ...
  contenedor.innerHTML = `
        ...
        <div class="overflow-auto">
          <table class="min-w-full text-sm">
            ...
            <tbody>
              ...
              ${filaConciliacion('Domicilios totales del día', '-', totalDomicilios, 'Se descuenta el costo completo de todos los envíos para que la caja refleje solo la operación del negocio.')}
      ...
      <div class="rounded-2xl border border-gray-200 overflow-hidden bg-white">
        ...
        <div class="p-4 space-y-3 text-sm">
          ...
          <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-2">
            ...
            <p><strong>Domicilios totales descontados:</strong> ${formatearCOP(totalDomicilios)} <span class="text-xs text-gray-500">(todos los costos de envío se excluyen del cierre operativo)</span></p>
    ...
    <div class="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
      ...
      <p class="mt-1">El domicilio nunca se cuenta como utilidad. Primero se calcula la venta real así: total cobrado a clientes menos domicilios. Luego se calcula la caja física así: apertura + efectivo recibido - costo total de domicilios. Así ningún valor de envío queda contado como venta o como dinero disponible del negocio.</p>
    </div>
  ...
  contenedor.innerHTML = `
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-3">
      ...
      <div class="rounded-xl border border-red-100 bg-red-50 p-3">
        <p class="text-xs uppercase tracking-wide text-gray-500">Domicilios descontados</p>
        <p class="text-lg font-extrabold text-red-700 mt-1">${formatearCOP(resumen.totalDomicilios || resumen.domiciliosDescontados || 0)}</p>
        <p class="text-xs text-gray-500 mt-1">Costo total de los envíos excluido del cierre</p>
      </div>
      <div class="rounded-xl border border-green-100 bg-green-50 p-3">
        <p class="text-xs uppercase tracking-wide text-gray-500">Cierre real del día</p>
        <p class="text-lg font-extrabold text-green-700 mt-1">${formatearCOP(cierreRealDia)}</p>
        <p class="text-xs text-gray-500 mt-1">Efectivo + transferencias - domicilios totales</p>
      </div>
      <div class="rounded-xl border border-yellow-100 bg-yellow-50 p-3">
        ...
        <p class="text-xs text-gray-500 mt-1">Apertura + efectivo - domicilios totales</p>
      </div>
    </div>
...
function programarMigracionAlmacenamientoC98() {
  const ejecutar = () => {
    try {
      ...
      console.warn('[Señor Arepa C9.39] La migración local se omitió sin bloquear el sistema:', error);
    }
  };
...
function configurarFirestoreRedRobusta(db) {
  ...
  try {
    ...
    console.info('[Firebase C9.39] Transporte robusto activado (long polling).');
  } catch (error) {
    const mensaje = String(error?.message || error || '');
    if (/already been started|settings can no longer be changed|failed-precondition/i.test(mensaje)) {
      console.warn('[Firebase C9.39] Firestore ya estaba iniciado; se conservaron sus ajustes actuales.');
    } else {
      console.warn('[Firebase C9.39] No fue posible aplicar los ajustes de red:', error);
    }
  }
  return db;
}
...
async function reconectarFirestoreSeguro(motivo = 'manual') {
  ...
  try {
    ...
    registrarHeartbeatFirebase();
    console.info(`[Firebase C9.39] Conexión restablecida (${motivo}).`);
    return true;
  } catch (error) {
    console.warn(`[Firebase C9.39] No se pudo restablecer la conexión (${motivo}):`, error);
    return false;
  } finally {
...
async function diagnosticarConexionFirebaseC98(opciones = {}) {
  ...
  if (!silencioso) {
    ...
    if (resultado.rest && resultado.sdk) {
      console.info('[Firebase C9.39] Conexión completa verificada.',
      ...
      console.error('[Firebase C9.39] Firebase responde por REST, pero el canal del SDK está bloqueado por red, antivirus, proxy o extensión.', resultado);
    } else {
      console.error('[Firebase C9.39] No se pudo verificar Firestore.', resultado);
    }
  }
  return resultado;
}
...
function ordenarVentasDesc(ventas) {
  return [...ventas].sort((a, b) => {
    const diaA = String(a?.diaClave || a?.diaClaveOficial || '');
    const diaB = String(b?.diaClave || b?.diaClaveOficial || '');
    if (diaA && diaB && diaA !== diaB) return diaB.localeCompare(diaA);
    if (diaA && diaB && diaA === diaB) {
      const numeroA = Number(a?.ordenDia || a?.numeroVentaDia || 0);
      const numeroB = Number(b?.ordenDia || b?.numeroVentaDia || 0);
      if (Number.isFinite(numeroA) && numeroA > 0 && Number.isFinite(numeroB) && numeroB > 0 && numeroA !== numeroB) {
        return numeroB - numeroA;
      }
    }
    const fechaA = a?.fechaNegocioISO || a?.fechaISO || a?.fechaOficialISO || a?.fecha || "";
    const fechaB = b?.fechaNegocioISO || b?.fechaISO || b?.fechaOficialISO || b?.fecha || "";
...
function cancelarPedidoActual() {
  ...
  async function guardarVenta() {
    ...
    const numeracionDia = comandaEnEdicion
      ...
      : { comanda: 0, recibo: 0, temporal: true };
    ...
    if (popupImpresionCocina) {
      try {
        popupImpresionCocina.document.write('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><title>Preparando comanda</title></head><body style="font-family:sans-serif;padding:16px;">Preparando comanda para cocina...</body></html>');
  ...
  function construirFilaVenta(v, i, numeroDia = null) {
    const resumen = {};
    (v.pedido || []).forEach(p => {
    ...
    const productosResumen = Object.entries(resumen)
      .map(([prod, cant]) => `<span class="sa-sales-product-chip"><span>${prod}</span><strong>x${cant}</strong></span>`)
      .join("");

    const botonesAccion = `
      <div class="sa-sales-actions">
        <button onclick="imprimirVentaCliente(${i})" class="sa-sales-action-button sa-sales-action-client" title="Imprimir recibo del cliente">🧾 Cliente</button>
        <button onclick="imprimirVentaCocina(${i})" class="sa-sales-action-button sa-sales-action-kitchen" title="Imprimir pedido para cocina">👨‍🍳 Cocina</button>
        ${ventaCancelada
          ? `<div class="sa-sales-action-status">${obtenerBadgeEstadoVenta(v)}</div>`
          : (tieneAccesoGestion() ? `<button onclick="marcarVentaComoCancelada(${i})" class="sa-sales-action-button sa-sales-action-cancel" title="Marcar la venta como cancelada">❌ Cancelar</button>` : "")}
        ${esAdmin() ? `<button onclick="editarVenta(${i})" class="sa-sales-action-button sa-sales-action-edit" title="Editar esta venta">📝 Editar</button>` : ""}
        ${esAdmin() ? `<button onclick="eliminarVentaPermanentemente(${i})" class="sa-sales-action-button sa-sales-action-delete" title="Eliminar definitivamente esta venta">🗑️ Eliminar</button>` : ""}
      </div>
    `;

    return `
      <tr class="${ventaCancelada ? 'sa-sales-row-cancelled' : ''}">
        <td class="sa-sales-select-column" style="display:${esAdmin() ? 'table-cell' : 'none'}">
          <input type="checkbox" ${ventaSeleccionada ? 'checked' : ''} onchange="toggleSeleccionVenta('${ventaKey}', this.checked)" class="w-4 h-4 accent-red-700 cursor-pointer" aria-label="Seleccionar venta ${numeroDia || obtenerNumeroVentaDiaPresentacion(v, i + 1)}" />
        </td>
        <td class="sa-sales-number-cell">${numeroDia || obtenerNumeroVentaDiaPresentacion(v, i + 1)}</td>
        <td class="sa-sales-date-cell">${formatearFechaHoraColombia(v.fechaNegocioISO || v.fechaISO || v.fecha)}</td>
        <td class="sa-sales-client-cell">${v.cliente || 'Sin cliente'}</td>
        <td>${obtenerEtiquetaFormaPago(v)}</td>
        <td>${formatearTipoPedidoVisual(v)}</td>
        <td>${obtenerBadgeEstadoVenta(v)}</td>
        <td><div class="sa-sales-products-list">${productosResumen || '<span class="text-gray-400">Sin productos</span>'}</div></td>
        <td class="sa-sales-total-cell">${formatearCOP(v.total)}</td>
        <td>${botonesAccion}</td>
      </tr>
    `;
  }
  ...
  function construirTarjetaVenta(v, i) {
    ...
    let botonesAccion = `
      <div class="grid grid-cols-2 gap-2 mt-3">
        <button onclick="imprimirVentaCliente(${i})" class="sa-sales-action-button sa-sales-action-client">🧾 Cliente</button>
        <button onclick="imprimirVentaCocina(${i})" class="sa-sales-action-button sa-sales-action-kitchen">👨‍🍳 Cocina</button>
      </div>
    `;

    if (!ventaCancelada && tieneAccesoGestion()) {
      botonesAccion += `<button onclick="marcarVentaComoCancelada(${i})" class="sa-sales-action-button sa-sales-action-cancel w-full mt-2">❌ Marcar como cancelada</button>`;
    }

    if (esAdmin()) {
      botonesAccion += `
        <div class="grid grid-cols-2 gap-2 mt-2">
          <button onclick="editarVenta(${i})" class="sa-sales-action-button sa-sales-action-edit">📝 Editar</button>
          <button onclick="eliminarVentaPermanentemente(${i})" class="sa-sales-action-button sa-sales-action-delete">🗑️ Eliminar</button>
        </div>
      `;
    }

    return `
      <article class="sa-sales-mobile-card ${ventaCancelada ? 'sa-sales-mobile-cancelled' : ''}">
        <div class="sa-sales-mobile-card-header">
          <div>
            ${esAdmin() ? `<label class="inline-flex items-center gap-2 text-xs font-bold text-gray-600 mb-2"><input type="checkbox" ${ventaSeleccionada ? 'checked' : ''} onchange="toggleSeleccionVenta('${ventaKey}', this.checked)" class="w-4 h-4 accent-red-700 cursor-pointer" /> Seleccionar</label>` : ''}
            <p class="text-xs font-bold text-amber-700">Venta #${obtenerNumeroVentaDiaPresentacion(v, i + 1)}</p>
            <h4 class="mt-1 font-black text-gray-800">${v.cliente || 'Sin cliente'}</h4>
          </div>
          <div class="text-right">
            <p class="text-base font-black ${ventaCancelada ? 'text-red-700 line-through' : 'text-green-700'}">${formatearCOP(obtenerIngresoRealVenta(v))}</p>
            <div class="mt-1">${obtenerBadgeEstadoVenta(v)}</div>
          </div>
        </div>
        <div class="sa-sales-mobile-card-body">
          <div class="sa-sales-mobile-metrics">
            <div class="sa-sales-mobile-metric"><span class="sa-sales-mobile-label">Fecha</span><span class="sa-sales-mobile-value">${formatearFechaHoraColombia(v.fechaISO || v.fecha)}</span></div>
            <div class="sa-sales-mobile-metric"><span class="sa-sales-mobile-label">Pago</span><span class="sa-sales-mobile-value">${obtenerEtiquetaFormaPago(v)}</span></div>
            <div class="sa-sales-mobile-metric sa-sales-mobile-metric-wide"><span class="sa-sales-mobile-label">Tipo de pedido</span><span class="sa-sales-mobile-value">${formatearTipoPedidoVisual(v)}</span></div>
            ${Number(v.costoDomicilio || 0) > 0 ? `<div class="sa-sales-mobile-metric sa-sales-mobile-metric-wide"><span class="sa-sales-mobile-label">Costo domicilio</span><span class="sa-sales-mobile-value">${formatearCOP(v.costoDomicilio)}</span></div>` : ''}
            ${(v.observaciones || '').trim() ? `<div class="sa-sales-mobile-metric sa-sales-mobile-metric-wide"><span class="sa-sales-mobile-label">Observaciones</span><span class="sa-sales-mobile-value">${v.observaciones}</span></div>` : ''}
          </div>
          <div class="sa-sales-mobile-products">
            <span class="sa-sales-mobile-label">Productos</span>
            <ul class="text-sm text-gray-700">${productosResumen || '<li>Sin productos</li>'}</ul>
          </div>
          ${botonesAccion}
        </div>
      </article>
  ...
  function obtenerNumeroDiarioVenta(venta = {}) {
    const numeroDia = Number(venta?.numeroVentaDia || 0);
    if (Number.isFinite(numeroDia) && numeroDia > 0) return Math.floor(numeroDia);
    const comanda = Number(venta?.comanda || 0);
    if (Number.isFinite(comanda) && comanda > 0) return Math.floor(comanda);
    const recibo = Number(venta?.recibo || 0);
    return Number.isFinite(recibo) && recibo > 0 ? Math.floor(recibo) : 0;
  }

  function obtenerMarcaTiempoVenta(venta = {}) {
    const candidatos = [venta?.fechaNegocioISO, venta?.fechaISO, venta?.fechaOficialISO, venta?.fechaClienteISO, venta?.fecha];
  ...
  function obtenerIdentidadVentaVisual(venta = {}, index = -1) {
    return String(venta?._docId || venta?._localId || `${venta?.diaClave || ''}|${venta?.fechaISO || venta?.fecha || ''}|${venta?.cliente || ''}|${venta?.totalCobrado ?? venta?.total ?? ''}|${index}`);
  }

  function compararVentasCronologicamente(a, b) {
    const tiempoA = obtenerMarcaTiempoVenta(a.venta);
    const tiempoB = obtenerMarcaTiempoVenta(b.venta);
    if (tiempoA !== tiempoB) return tiempoA - tiempoB;
    return obtenerIdentidadVentaVisual(a.venta, a.index).localeCompare(obtenerIdentidadVentaVisual(b.venta, b.index));
  }

  function obtenerVentasDelDiaActualConIndices() {
    const hoy = obtenerClaveDiaActual();
    const ventas = obtenerVentasStorage();
    const cronologicas = ventas
      ...
      .sort(compararVentasCronologicamente);

    // C9.39: las ventas schemaVersion 6 usan el consecutivo oficial guardado
    // por Firestore. Los documentos heredados se reconstruyen solo para
    // presentación; una venta pendiente NO recibe un número ficticio.
    return cronologicas
      .map((entrada, posicion) => {
        const persistido = obtenerNumeroDiarioVenta(entrada.venta);
        const oficialV6 = Number(entrada.venta?.schemaVersion || 0) >= 6
          && !entrada.venta?.consecutivoTemporal
          && persistido > 0;
        const pendiente = entrada.venta?._syncEstado === 'pendiente' || entrada.venta?.consecutivoTemporal;
        return {
          ...entrada,
          numeroDia: oficialV6 ? persistido : (pendiente ? 'Pendiente' : posicion + 1),
          ordenVisual: oficialV6 ? persistido : posicion + 1
        };
      })
      .sort((a, b) => Number(b.ordenVisual || 0) - Number(a.ordenVisual || 0));
  }

  function obtenerNumeroVentaDiaPresentacion(venta = {}, referencia = 1) {
    const persistido = Number(venta?.numeroVentaDia || 0);
    const esOficialAtomica = Number(venta?.schemaVersion || 0) >= 6
      && !venta?.consecutivoTemporal
      && Number.isFinite(persistido)
      && persistido > 0;
    if (esOficialAtomica) return Math.floor(persistido);
    try {
      const fechaBase = venta?.fechaISO ? new Date(venta.fechaISO) : new Date(venta?.fecha || Date.now());
      const dia = venta?.diaClave || obtenerFechaLocalISO(fechaBase);
      const identidad = obtenerIdentidadVentaVisual(venta);
      const lista = obtenerVentasStorage()
        .map((item, index) => ({ venta: item, index }))
        .filter(({ venta: item }) => {
          const f = item?.fechaISO ? new Date(item.fechaISO) : new Date(item?.fecha || Date.now());
          return (item?.diaClave || obtenerFechaLocalISO(f)) === dia;
        })
        .sort(compararVentasCronologicamente);
      const posicion = lista.findIndex(({ venta: item, index }) => obtenerIdentidadVentaVisual(item, index) === identidad ||
        (venta?._docId && item?._docId === venta._docId) ||
        (venta?._localId && item?._localId === venta._localId));
      if (posicion >= 0) return posicion + 1;
    } catch (_) {}
    const guardado = obtenerNumeroDiarioVenta(venta);
    if (Number(venta?.schemaVersion || 0) >= 6 && !venta?.consecutivoTemporal && guardado > 0) return guardado;
    return Math.max(1, Number(referencia || 1));
  }
  ...
  function renderVentasTabla() {
    ...
    contenedor.innerHTML = ventasPagina.length
      ? ventasPagina.map(({ venta, index, numeroDia }) => construirFilaVenta(venta, index, numeroDia)).join("")
      : `<tr><td colspan="10" class="p-6 text-center text-gray-500">No hay ventas del día para mostrar.</td></tr>`;
...
function generarPlantillaReciboCliente(venta = {}, referencia = 1) {
  ...
  <html lang="es">
    ...
    <body>
      ...
      <div class="line"></div>
      <div class="info">
        <p class="numero">RECIBO #${obtenerNumeroVentaDiaPresentacion(venta, referencia)}</p>
        <p class="numero">COMANDA #${obtenerNumeroVentaDiaPresentacion(venta, referencia)}</p>
