...
const SENOR_AREPA_STORAGE_VERSION = 'C9.29';
...
function leerLocalStorageJSONSeguro(key, fallback) {
  try {
    ...
    console.warn(`[Storage C9.29] Se descartó una copia local dañada: ${key}.`, error);
...
function guardarLocalStorageSeguro(key, value, opciones = {}) {
  ...
  try {
    ...
    console.error(`[Storage C9.29] No se pudo serializar ${key}:`, error);
    return false;
  }
  try {
    ...
    if (!esErrorCuotaStorage(error)) {
      console.error(`[Storage C9.29] No se pudo guardar ${key}:`, error);
      return false;
    }
    ...
    try {
      localStorage.setItem(key, texto);
      console.warn(
        `[Storage C9.29] Se liberó espacio automáticamente para ${key}.`,
      ...
      if (critico) {
        console.error(`[Storage C9.29] ${mensaje}`, segundoError);
        try {
        ...
        console.warn(`[Storage C9.29] ${mensaje}`, segundoError);
      }
      return false;
    }
  }
...
function migrarAlmacenamientoLocalC95() {
  try {
    ...
    console.info('[Señor Arepa C9.29] Almacenamiento local optimizado.', estimarUsoLocalStorage());
  } catch (error) {
    console.warn('[Señor Arepa C9.29] No se pudo completar la migración de almacenamiento:', error);
  }
}
...
window.limpiarCacheLocalSenorArepa = function limpiarCacheLocalSenorArepa() {
  ...
  try { localStorage.removeItem(ULTIMA_VENTA_GUARDADA_KEY); } catch (_) {}
  console.info('[Señor Arepa C9.29] Limpieza manual terminada.', estimarUsoLocalStorage());
...
function desactivarFirebaseCajaPorPermisos(error) {
  if (!firebaseAuth?.currentUser) {
    console.info('[Firebase C9.29] Se omitió una consulta de controlCaja porque la sesión todavía no estaba autenticada.');
    return;
  }
  ...
  if (!controlCajaFirebasePermisosAvisados) {
    ...
    console.warn(
      `Control de caja funcionando temporalmente en modo local. Firestore rechazó controlCaja para ${emailSesion} en ${projectIdSesion}. Publica firestore.rules C9.29.`,
      error
    );
    if (typeof window.notificarSistema === 'function') {
      window.notificarSistema(`Firestore rechazó controlCaja para ${emailSesion}. Publica las reglas C9.29.`, 'error');
    }
  }
...
function programarMigracionAlmacenamientoC98() {
  const ejecutar = () => {
    try {
      ...
      console.warn('[Señor Arepa C9.29] La migración local se omitió sin bloquear el sistema:', error);
    }
  };
...
function configurarFirestoreRedRobusta(db) {
  ...
  try {
    ...
    console.info('[Firebase C9.29] Transporte robusto activado (long polling).');
  } catch (error) {
    const mensaje = String(error?.message || error || '');
    if (/already been started|settings can no longer be changed|failed-precondition/i.test(mensaje)) {
      console.warn('[Firebase C9.29] Firestore ya estaba iniciado; se conservaron sus ajustes actuales.');
    } else {
      console.warn('[Firebase C9.29] No fue posible aplicar los ajustes de red:', error);
    }
  }
  return db;
}
...
async function reconectarFirestoreSeguro(motivo = 'manual') {
  ...
  try {
    ...
    registrarHeartbeatFirebase();
    console.info(`[Firebase C9.29] Conexión restablecida (${motivo}).`);
    return true;
  } catch (error) {
    console.warn(`[Firebase C9.29] No se pudo restablecer la conexión (${motivo}):`, error);
    return false;
  } finally {
...
async function diagnosticarConexionFirebaseC98(opciones = {}) {
  ...
  if (!silencioso) {
    ...
    if (resultado.rest && resultado.sdk) {
      console.info('[Firebase C9.29] Conexión completa verificada.',
      ...
      console.error('[Firebase C9.29] Firebase responde por REST, pero el canal del SDK está bloqueado por red, antivirus, proxy o extensión.', resultado);
    } else {
      console.error('[Firebase C9.29] No se pudo verificar Firestore.', resultado);
    }
  }
  return resultado;
}
...
function cancelarPedidoActual() {
  ...
  function obtenerVentasDelDiaActualConIndices() {
    ...
    // C9.29: las ventas schemaVersion 5 usan el consecutivo oficial guardado
    ...
    return cronologicas
      .map((entrada, posicion) => {
        ...
        const oficialV5 = Number(entrada.venta?.schemaVersion || 0) >= 5
          && !entrada.venta?.consecutivoTemporal
          && persistido > 0;
        const pendiente = entrada.venta?._syncEstado === 'pendiente' || entrada.venta?.consecutivoTemporal;
        return {
          ...entrada,
          numeroDia: oficialV5 ? persistido : (pendiente ? 'Pendiente' : posicion + 1),
          ordenVisual: oficialV5 ? persistido : posicion + 1
        };
      })
  ...
  function obtenerNumeroVentaDiaPresentacion(venta = {}, referencia = 1) {
    ...
    return guardado > 0 ? guardado : Math.max(1, Number(referencia || 1));
  }
