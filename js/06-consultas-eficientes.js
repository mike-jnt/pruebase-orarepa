(() => {
  ...
  const VERSION = '2026.08.08-C9.29-CIERRES';
  const PAGE_SIZE = 10;
  ...
  async function recuperarVentasLegadasSiFaltan(today, actuales) {
    ...
    try {
      ...
      console.warn(`[C9.29] El resumen diario reporta ${esperado} venta(s), pero las consultas directas encontraron ${actuales.length}. Se activa recuperación compatible.`);
      ...
      if (recuperadas.length > actuales.length) {
        console.info(`[C9.29] Recuperación diaria completada: ${recuperadas.length} venta(s) localizadas.`);
      }
      return recuperadas;
    } catch (error) {
      console.warn('[C9.29] No fue posible ejecutar la recuperación compatible de ventas del día:', error);
      return actuales;
    } finally {
  ...
  function programarPublicacionVentasDia(today) {
    ...
    publicarVentasDiaTimer = setTimeout(async () => {
      ...
      if (typeof reconciliarContadorVentasDia === 'function' && online()) {
        await reconciliarContadorVentasDia(today, sales);
      }
      ...
      console.info(`[C9.29] Ventas del día cargadas: ${visibleTodayCount()} registro(s) · ${totalPages} página(s).`);
  ...
  escucharVentasFirestore = function() {
    ...
    // C9.29: se unen DOS consultas para no perder ventas heredadas.
    ...
    const unsubDayKey = queryDayKey.onSnapshot(snapshot => {
      ...
      state.today.readyDayKey = true;
      console.error('[C9.29] No se pudo consultar ventas por diaClave:', error);
      programarPublicacionVentasDia(today);
    });
    ...
    const unsubDateRange = queryDateRange.onSnapshot(snapshot => {
      ...
      state.today.readyDateRange = true;
      console.error('[C9.29] No se pudo consultar ventas por fechaISO:', error);
      programarPublicacionVentasDia(today);
    });
  ...
  window.asegurarIntegridadVentasDiaAntesDeGuardar = async function(dia = dayKey()) {
    ...
    preparacionDiaVentas.promise = (async () => {
      try {
        const range = dayRange(dia);
        const [byDay, byDate] = await Promise.all([
          firestoreDb.collection('ventas').where('diaClave', '==', dia).limit(DAILY_SALES_LIMIT).get(),
        ...
        const map = new Map();
        [...byDay.docs, ...byDate.docs].forEach(doc => {
          const sale = normalizeSale(doc);
          if (saleDay(sale) === dia) map.set(saleId(sale), sale);
        });
        const sales = Array.from(map.values());
        mergeSalesIntoCache(sales, dia === dayKey());
        if (typeof reconciliarContadorVentasDia === 'function') {
          const counter = await reconciliarContadorVentasDia(dia, sales);
          if (!counter) throw new Error('No fue posible reconciliar el contador diario.');
        }
        preparacionDiaVentas.preparadoEn = Date.now();
        preparacionDiaVentas.total = sales.length;
        console.info(`[C9.29] Día ${dia} preparado antes de guardar: ${sales.length} venta(s) reales reconciliadas.`);
        return { ok: true, diaClave: dia, total: sales.length };
      } catch (error) {
        preparacionDiaVentas.preparadoEn = 0;
        console.error(`[C9.29] No se pudo preparar el día ${dia} antes de guardar:`, error);
  ...
  // Diagnóstico de integridad C9.29: permite comprobar desde consola que todas
  // las ventas del día comparten diaClave y que los consecutivos oficiales no
  // tienen duplicados ni huecos entre los documentos schemaVersion 5.
  window.diagnosticarIntegridadVentasDia = function(dia = dayKey()) {
    const sales = (typeof obtenerVentasStorage === 'function' ? obtenerVentasStorage() : [])
      .filter(sale => saleDay(sale) === dia);
    const oficiales = sales
      .filter(sale => Number(sale?.schemaVersion || 0) >= 5 && !sale?.consecutivoTemporal)
    ...
    const resultado = {
      version: 'C9.29', diaClave: dia, ventasCargadas: sales.length,
      ventasOficialesV5: oficiales.length, consecutivos: oficiales,
  ...
  window.diagnosticarVentasDiaActual = function diagnosticarVentasDiaActual() {
    ...
    if (duplicates.length) console.warn('[C9.29] Se detectaron números heredados duplicados. La tabla usa la secuencia cronológica diaria corregida.', duplicates);
    return report;
  };
  ...
  async function obtenerResumenFinancieroDia(diaClave) {
    try {
      ...
      console.warn(`[C9.29] No se pudo consultar el resumen diario de ${diaClave}:`, error);
    }
    ...
    try {
      ...
      console.warn(`[C9.29] No se pudo reconstruir la caja desde las ventas de ${diaClave}:`, error);
    }
    return null;
  }
  ...
  async function fetchCashPage(reset = false) {
    ...
    try {
      ...
      if (requiereIndice) {
        console.error('[C9.29] Falta o está construyéndose el índice de controlCaja por diaClave DESCENDING. Despliega firestore.indexes.json.', error);
      } else {
        console.error('[C9.29] No se pudieron cargar los cierres desde Firestore:', error);
      }
