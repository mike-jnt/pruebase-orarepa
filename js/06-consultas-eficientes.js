(() => {
  ...
  const VERSION = '2026.08.06-C9.27-CIERRES';
  const PAGE_SIZE = 10;
  ...
  escucharVentasFirestore = function() {
    ...
    const today = dayKey();
    // C9.27: se escucha el día completo y la paginación se hace localmente.
    // Así una venta nueva no reemplaza las páginas anteriores ni desaparecen números.
    ...
    state.today.unsubscribe = query.onSnapshot(snapshot => {
      ...
      state.today.hasMore = snapshot.size >= DAILY_SALES_LIMIT;
      state.today.search = '';
      ...
      state.today.loading = false;
      console.error('Error al cargar todas las ventas del día:', error);
  ...
  async function obtenerResumenFinancieroDia(diaClave) {
    try {
      ...
      console.warn(`[C9.27] No se pudo consultar el resumen diario de ${diaClave}:`, error);
    }
    ...
    try {
      ...
      console.warn(`[C9.27] No se pudo reconstruir la caja desde las ventas de ${diaClave}:`, error);
    }
    return null;
  }
  ...
  async function fetchCashPage(reset = false) {
    ...
    try {
      ...
      if (requiereIndice) {
        console.error('[C9.27] Falta o está construyéndose el índice de controlCaja por diaClave DESCENDING. Despliega firestore.indexes.json.', error);
      } else {
        console.error('[C9.27] No se pudieron cargar los cierres desde Firestore:', error);
      }
