(() => {
  ...
  const VERSION = '2026.08.04-C9.21-CIERRES';
  const PAGE_SIZE = 10;
  ...
  async function obtenerResumenFinancieroDia(diaClave) {
    try {
      ...
      console.warn(`[C9.21] No se pudo consultar el resumen diario de ${diaClave}:`, error);
    }
    ...
    try {
      ...
      console.warn(`[C9.21] No se pudo reconstruir la caja desde las ventas de ${diaClave}:`, error);
    }
    return null;
  }
  ...
  async function fetchCashPage(reset = false) {
    ...
    try {
      ...
      if (requiereIndice) {
        console.error('[C9.21] Falta o está construyéndose el índice de controlCaja por diaClave DESCENDING. Despliega firestore.indexes.json.', error);
      } else {
        console.error('[C9.21] No se pudieron cargar los cierres desde Firestore:', error);
      }
