(() => {
  ...
  const VERSION = '2026.09.06-C9.39-CIERRES';
  const PAGE_SIZE = 10;
  ...
  async function recuperarVentasLegadasSiFaltan(today, actuales) {
    ...
    try {
      ...
      console.warn(`[C9.39] El resumen diario reporta ${esperado} venta(s), pero las consultas directas encontraron ${actuales.length}. Se activa recuperación compatible.`);
      ...
      if (recuperadas.length > actuales.length) {
        console.info(`[C9.39] Recuperación diaria completada: ${recuperadas.length} venta(s) localizadas.`);
      }
      return recuperadas;
    } catch (error) {
      console.warn('[C9.39] No fue posible ejecutar la recuperación compatible de ventas del día:', error);
      return actuales;
    } finally {
  ...
  function programarPublicacionVentasDia(today) {
    ...
    publicarVentasDiaTimer = setTimeout(async () => {
      ...
      // C9.39: cargar la tabla NO prepara el contador. El contador pertenece a
      // la apertura de caja y se valida únicamente al abrir jornada o guardar.
      ...
      console.info(`[C9.39] Ventas del día cargadas: ${visibleTodayCount()} registro(s) · ${totalPages} página(s).`);
  ...
  escucharVentasFirestore = function() {
    ...
    // C9.39: se unen DOS consultas para no perder ventas heredadas.
    ...
    const unsubDayKey = queryDayKey.onSnapshot(snapshot => {
      ...
      state.today.readyDayKey = true;
      console.error('[C9.39] No se pudo consultar ventas por diaClave:', error);
      programarPublicacionVentasDia(today);
    });
    ...
    const unsubDateRange = queryDateRange.onSnapshot(snapshot => {
      ...
      state.today.readyDateRange = true;
      console.error('[C9.39] No se pudo consultar ventas por fechaISO:', error);
      programarPublicacionVentasDia(today);
    });
  ...
  window.asegurarIntegridadVentasDiaAntesDeGuardar = async function(dia = dayKey()) {
    ...
    preparacionDiaVentas.promise = (async () => {
      try {
        // C9.39: primero se confirma que exista una apertura REMOTA. Así no se
        // genera ruido de contador ni se intenta guardar una venta sin jornada.
        let control = null;
        if (!firestoreDb) {
          return { ok: false, requiereApertura: true, diaClave: dia, total: loadedTodayCount(), error: 'Firestore no está disponible para confirmar la apertura.' };
        }
        const aperturaSnap = await firestoreDb.collection('controlCaja').doc(dia).get();
        control = aperturaSnap.exists ? { diaClave: dia, ...aperturaSnap.data() } : null;
        if (!control?.aperturaHora) {
          return { ok: false, requiereApertura: true, diaClave: dia, total: loadedTodayCount(), error: 'La caja aún no tiene apertura sincronizada.' };
        }
        if (typeof prepararContadorAtomicoVentasDia !== 'function') {
          throw new Error('No está disponible el contador atómico de ventas.');
        }
        const counter = await prepararContadorAtomicoVentasDia(dia, { control });
        ...
        console.error(`[C9.39] No se pudo preparar el día ${dia} antes de guardar:`, error);
  ...
  // Diagnóstico de integridad C9.39: permite comprobar desde consola que todas
  ...
  window.diagnosticarIntegridadVentasDia = function(dia = dayKey()) {
    ...
    const resultado = {
      version: 'C9.39', diaClave: dia, ventasCargadas: sales.length,
  ...
  window.diagnosticarVentasDiaActual = function diagnosticarVentasDiaActual() {
    ...
    if (duplicates.length) console.warn('[C9.39] Se detectaron números heredados duplicados. La tabla usa la secuencia cronológica diaria corregida.', duplicates);
    return report;
  };
  ...
  function moverDiaClave(day, deltaDays = 0) {
  ...
  function inicioSemanaDesdeDiaClave(day) {
    const match = String(day || '').match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!match) return day;
    const date = new Date(Date.UTC(Number(match[1]), Number(match[2]) - 1, Number(match[3])));
  ...
  function inicioMesDesdeDiaClave(day) {
    return `${String(day || dayKey()).slice(0, 7)}-01`;
  }

  function rangoIsoColombia(startDay, endDayInclusive) {
    const start = new Date(`${startDay}T05:00:00.000Z`);
    const endExclusive = new Date(`${moverDiaClave(endDayInclusive, 1)}T05:00:00.000Z`);
    return { start: start.toISOString(), endExclusive: endExclusive.toISOString() };
  }

  async function leerVentasHistoricoActual(startDay, endDay) {
    if (!online()) return [];
    const byId = new Map();
    const addSnapshot = snapshot => {
      snapshot.docs.forEach(doc => {
        const sale = normalizeSale(doc);
        const actualDay = saleDay(sale);
        if (actualDay >= startDay && actualDay <= endDay) byId.set(doc.id, sale);
      });
    };

    // Ruta oficial C9.29+: todas las ventas deben tener diaClave.
    const byDay = await firestoreDb.collection('ventas')
      .where('diaClave', '>=', startDay)
      .where('diaClave', '<=', endDay)
      .get();
    addSnapshot(byDay);

    // Compatibilidad: recupera ventas antiguas cuya fecha existe pero diaClave
    // estaba ausente o fue guardado por una versión anterior.
    const iso = rangoIsoColombia(startDay, endDay);
    try {
      const byDate = await firestoreDb.collection('ventas')
        .where('fechaISO', '>=', iso.start)
        .where('fechaISO', '<', iso.endExclusive)
        .get();
      addSnapshot(byDate);
    } catch (error) {
      console.warn('[C9.39] Histórico: no fue posible ejecutar la recuperación por fechaISO; se conserva la consulta oficial por diaClave.', error);
    }
    return Array.from(byId.values());
  }

  function resumirVentasHistorico(sales = []) {
    return sales.reduce((acc, saleRaw) => {
      const sale = normalizeSale(saleRaw || {});
      const cancelled = String(sale.estado || '').toLowerCase() === 'cancelada';
      const delivery = String(sale.tipoPedido || '').toLowerCase() === 'domicilio';
      const productsFromItems = (sale.pedido || []).reduce((sum, product) => {
        const quantity = Math.max(1, Number(product?.cantidad || 1));
        return sum + Number(product?.precio || 0) * quantity;
      }, 0);
      const productTotal = Math.max(0, Number(sale.subtotalProductos ?? sale.total ?? productsFromItems) || productsFromItems);
      if (cancelled) {
        acc.pedidosCancelados += 1;
        if (delivery) acc.domiciliosCancelados += 1;
        return acc;
      }
      acc.pedidosActivos += 1;
      acc.totalVentas += productTotal;
      if (delivery) acc.domiciliosActivos += 1;
      return acc;
    }, { pedidosActivos: 0, pedidosCancelados: 0, domiciliosActivos: 0, domiciliosCancelados: 0, totalVentas: 0 });
  }

  async function renderHistoryCards() {
    const today = dayKey();
    const weekStart = inicioSemanaDesdeDiaClave(today);
    const monthStart = inicioMesDesdeDiaClave(today);
    ...
    const apply = (summary, salesId, totalId) => {
      ...
      set(totalId, `${money(summary?.totalVentas || 0)} · ${Math.max(0, Number(summary?.domiciliosActivos || 0))} domicilio(s)`);
    };

    try {
      // C9.39: la fuente primaria de los tres cuadros son los documentos reales
      // de ventas. Los resúmenes dejan de ser un punto único de fallo.
      const sales = await leerVentasHistoricoActual(earliest, today);
      const daily = resumirVentasHistorico(sales.filter(sale => saleDay(sale) === today));
      const weekly = resumirVentasHistorico(sales.filter(sale => {
        const day = saleDay(sale);
        return day >= weekStart && day <= today;
      }));
      const monthly = resumirVentasHistorico(sales.filter(sale => {
        const day = saleDay(sale);
      ...
      set('histFuenteDatos', `Conectado a ventas reales de Firestore · ${sales.length} documento(s) revisado(s)`);
      return;
    } catch (error) {
      console.warn('[C9.39] Histórico: falló la lectura directa de ventas; se usarán los resúmenes como respaldo.', error);
    }

    // Respaldo compatible con documentos de resumen existentes.
    ...
    apply(weekly, 'histSemanaVentas', 'histSemanaTotal');
    apply(monthly, 'histMesVentas', 'histMesTotal');
    set('histFuenteDatos', 'Respaldo: documentos de resumen de Firestore');
  }
  ...
  async function obtenerResumenFinancieroDia(diaClave) {
    try {
      ...
      console.warn(`[C9.39] No se pudo consultar el resumen diario de ${diaClave}:`, error);
    }
    ...
    try {
      ...
      console.warn(`[C9.39] No se pudo reconstruir la caja desde las ventas de ${diaClave}:`, error);
    }
    return null;
  }
  ...
  async function fetchCashPage(reset = false) {
    ...
    try {
      ...
      if (requiereIndice) {
        console.error('[C9.39] Falta o está construyéndose el índice de controlCaja por diaClave DESCENDING. Despliega firestore.indexes.json.', error);
      } else {
        console.error('[C9.39] No se pudieron cargar los cierres desde Firestore:', error);
      }
