(() => {
  ...
  async function fetchSummaryPage(key, reset = false) {
    ...
    try {
      ...
      const snapshot = await query.limit(PAGE_SIZE).get();
      target.docs.push(...snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
  ...
  function renderSummaryTable(key) {
    ...
    if (!ids) return;
    const rows = target.docs.map(summaryRow).map(row => `<tr><td class="p-2 border">${row.label}</td><td class="p-2 border">${row.ventas}${row.canceladas ? ` activas / ${row.canceladas} canceladas` : ''}</td><td class="p-2 border">${row.domicilios}${row.domiciliosCancelados ? ` activos / ${row.domiciliosCancelados} cancelados` : ''}</td><td class="p-2 border">${money(row.total)}</td></tr>`);
  ...
  function patchGenericNextButton(key, target) {
    const ids = {
      historicoDia: ['btnNextHistoricoDia', 'infoPaginacionHistoricoDia'],
      historicoSemana: ['btnNextHistoricoSemana', 'infoPaginacionHistoricoSemana'],
      historicoMes: ['btnNextHistoricoMes', 'infoPaginacionHistoricoMes'],
      domiciliosDia: ['btnNextDomiciliosDia', 'infoPaginacionDomiciliosDia'],
      historicoDetalle: ['btnNextHistoricoDetalle', 'infoPaginacionHistoricoDetalle'],
      domiciliosDetalle: ['btnNextDomiciliosDetalle', 'infoPaginacionDomiciliosDetalle'],
      cierresCaja: ['btnNextCierresCaja', 'infoPaginacionCierresCaja']
    }[key];
    if (!ids) return;
    const next = document.getElementById(ids[0]);
    const info = document.getElementById(ids[1]);
    const pagesLoaded = Math.max(1, Math.ceil(target.docs.length / PAGE_SIZE));
    const current = obtenerEstadoPaginacionTabla(key).pagina;
    if (next) {
      next.disabled = target.loading || (current >= pagesLoaded && !target.hasMore);
      next.classList.toggle('opacity-50', next.disabled);
    }
    if (info && target.hasMore) info.textContent += ' · hay más registros en la base';
  }
  ...
  actualizarHistoricos = async function() {
    ...
    const report = document.getElementById('reportesAvanzadosCards');
    if (report) report.innerHTML = '<div class="sm:col-span-2 xl:col-span-5 rounded-xl border border-indigo-100 bg-white p-3 text-sm text-gray-600">Los indicadores avanzados se calculan con el periodo que consultes; no se descarga el histórico completo al abrir esta vista.</div>';
  };
  ...
  function renderDomicileDetail() {
    ...
    const active = sales.filter(s => String(s.estado || '').toLowerCase() !== 'cancelada');
    const summary = document.getElementById('resumenDomiciliosDiaSeleccionado');
    if (summary) summary.innerHTML = `<strong>Registros cargados:</strong> ${sales.length} &nbsp;|&nbsp; <strong>Activos:</strong> ${active.length} &nbsp;|&nbsp; <strong>Valor de domicilios cargado:</strong> ${money(active.reduce((sum, sale) => sum + Number(sale.costoDomicilio || 0), 0))}${target.hasMore ? ' &nbsp;|&nbsp; <span class="text-amber-700">Hay más registros; usa Siguiente.</span>' : ''}`;
  ...
  cambiarPaginaTabla = async function(key, direction) {
    ...
    const desired = Math.max(1, Number(pagination.pagina || 1) + Number(direction || 0));
    if (target) {
      const loadedPages = Math.max(1, Math.ceil(target.docs.length / PAGE_SIZE));
      if (direction > 0 && desired > loadedPages && target.hasMore) {
        if (key === 'cierresCaja') await fetchCashPage(false);
        else if (state.summaries[key]) await fetchSummaryPage(key, false);
        else await fetchDetailPage(key, false);
      }
      pagination.pagina = Math.min(desired, Math.max(1, Math.ceil(target.docs.length / PAGE_SIZE)));
    } else {
  ...
  reconstruirResumenesHistoricos = async function() {
  ...
  escucharInventarioFirestore = function() {
    ...
    inventarioUnsubscribe = firestoreDb.collection(INVENTARIO_COLLECTION)
      ...
      .onSnapshot(snapshot => {
        ...
        localStorage.setItem(INVENTARIO_STORAGE_KEY, JSON.stringify(rows));
