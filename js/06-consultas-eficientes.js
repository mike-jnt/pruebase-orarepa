(() => {
  ...
  const VERSION = '2026.08.04-C9.18-CIERRES';
  const PAGE_SIZE = 10;
  ...
  function tieneResumenFinancieroCaja(control = {}) {
    ...
    const tieneDomicilios = fuentes.some(fuente => tienePropiedadCaja(fuente, [
      'ajusteDomiciliosTransferencia', 'ajusteDomiciliosDigitales', 'domiciliosDescontados', 'domiciliosTransferencia'
    ]));
    return tieneEfectivo && tieneTransferencias && tieneDomicilios;
  }
  ...
  function convertirResumenDiarioACaja(control = {}, resumenDia = {}, origen = 'resumenVentasDiario') {
    ...
    const domiciliosDescontados = Number(
      resumenDia.domiciliosTransferenciaValor
      ?? resumenDia.ajusteDomiciliosTransferencia
      ?? resumenDia.domiciliosDescontados
      ?? 0
    );
    const efectivoNeto = Math.max(0, efectivo - domiciliosDescontados);
    ...
    return {
      ...
      resumenCierre: {
        ...
        transferencias,
        ajusteDomiciliosTransferencia: domiciliosDescontados,
        ajusteDomiciliosDigitales: domiciliosDescontados,
  ...
  async function obtenerResumenFinancieroDia(diaClave) {
    try {
      ...
      console.warn(`[C9.18] No se pudo consultar el resumen diario de ${diaClave}:`, error);
    }
    ...
    try {
      ...
      console.warn(`[C9.18] No se pudo reconstruir la caja desde las ventas de ${diaClave}:`, error);
    }
    return null;
  }
  ...
  async function fetchCashPage(reset = false) {
    ...
    try {
      ...
      if (requiereIndice) {
        console.error('[C9.18] Falta o está construyéndose el índice de controlCaja por diaClave DESCENDING. Despliega firestore.indexes.json.', error);
      } else {
        console.error('[C9.18] No se pudieron cargar los cierres desde Firestore:', error);
      }
