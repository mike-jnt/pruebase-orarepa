// --- Numeración diaria e históricos ---
  ...
  // C9.29: sello único de negocio para todas las ventas nuevas.
  // La fecha del pedido se fija una sola vez en zona horaria Colombia y se
  // conserva durante toda la vida del documento. Firestore añade además el
  // timestamp oficial del servidor al confirmar la transacción.
  function obtenerSelloVentaColombia(fecha = new Date()) {
    const fechaObj = fecha instanceof Date ? fecha : new Date(fecha);
    const valida = Number.isNaN(fechaObj.getTime()) ? new Date() : fechaObj;
    const partes = obtenerPartesColombia(valida);
    const milisegundos = String(valida.getMilliseconds()).padStart(3, '0');
    const diaClave = `${partes.year}-${partes.month}-${partes.day}`;
    const fechaNegocioISO = `${diaClave}T${partes.hour}:${partes.minute}:${partes.second}.${milisegundos}-05:00`;
    return {
      diaClave,
      fechaISO: valida.toISOString(),
      fechaNegocioISO,
      mesClave: diaClave.slice(0, 7),
      semanaClave: obtenerClaveSemana(valida),
      zonaHoraria: TIMEZONE_CO
    };
  }
  window.obtenerSelloVentaColombia = obtenerSelloVentaColombia;
  ...
  function obtenerConsecutivoNumerico(valor) {
    const numero = Number(valor || 0);
    return Number.isFinite(numero) && numero > 0 ? Math.floor(numero) : 0;
  }

  function obtenerSiguienteNumeracionDelDia(diaClave) {
    // C9.29: el consecutivo provisional nunca vuelve a empezar desde 1 solo
    // porque cambie el navegador. Se toma como base el mayor valor conocido
    // entre la memoria local y las ventas ya cargadas para ese día. El número
    // definitivo sigue siendo reservado por Firestore al sincronizar.
    const key = `numeracionDia_${diaClave}`;
    let actual = { comanda: 0, recibo: 0 };
    try { actual = JSON.parse(localStorage.getItem(key) || '{"comanda":0,"recibo":0}') || actual; } catch (_) {}

    let ventasDia = [];
    try {
      if (typeof obtenerVentasStorage === 'function') {
        ventasDia = obtenerVentasStorage().filter(venta => {
          ...
          return diaVenta === diaClave;
        });
      }
    } catch (_) {}

    const maxComanda = ventasDia.reduce((max, venta) => Math.max(max, obtenerConsecutivoNumerico(venta?.numeroVentaDia), obtenerConsecutivoNumerico(venta?.comanda)), 0);
    const maxRecibo = ventasDia.reduce((max, venta) => Math.max(max, obtenerConsecutivoNumerico(venta?.numeroVentaDia), obtenerConsecutivoNumerico(venta?.recibo)), 0);
    const totalConocido = ventasDia.length;
    const siguiente = {
      comanda: Math.max(obtenerConsecutivoNumerico(actual.comanda), maxComanda, totalConocido) + 1,
      recibo: Math.max(obtenerConsecutivoNumerico(actual.recibo), maxRecibo, totalConocido) + 1
    };
  ...
  function normalizarVenta(v, index) {
    ...
    return {
      ...
      diaClave,
      // C9.29: no se inventan números a partir del índice del arreglo.
      // Los documentos heredados sin número se muestran con la secuencia
      // cronológica diaria calculada por el POS, sin alterar sus datos.
      numeroVentaDia: obtenerConsecutivoNumerico(v.numeroVentaDia) || null,
      comanda: v.comanda ?? v.recibo ?? null,
      recibo: v.recibo ?? v.comanda ?? null,
      subtotalProductos,
