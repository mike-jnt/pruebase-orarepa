// --- Numeración diaria e históricos ---
  ...
  // C9.39: sello único de negocio para todas las ventas nuevas.
  ...
  function normalizarVenta(v, index) {
    const fechaBase = v.fechaISO ? new Date(v.fechaISO) : (v.fecha ? new Date(v.fecha) : null);
    const fechaValida = fechaBase && !isNaN(fechaBase.getTime()) ? fechaBase : null;
    // C9.39: una venta vieja sin fecha válida NO se convierte silenciosamente
    // en una venta de hoy. El día queda vacío hasta que pueda verificarse.
    const diaClave = v.diaClave || (fechaValida ? obtenerFechaLocalISO(fechaValida) : '');
    ...
    return {
      ...
      fechaISO: v.fechaISO || (fechaValida ? fechaValida.toISOString() : ''),
      diaClave,
      // C9.39: no se inventan números a partir del índice del arreglo.
